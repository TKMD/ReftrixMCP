...
>
> Token Bucket rate limiting is implemented (v0.2.0) with 3 tiers. Uses atomic Redis Lua scripts when Redis is available, falls back to in-memory buckets otherwise. Configure default tier RPM via `RATE_LIMIT_RPM` env var. Disable with `RATE_LIMIT_ENABLED=false`.
...
**A:** ReftrixはMCPサーバー + スタンドアロンCLI構成です。`layout.generate_code` で生成したコードを表示する場合は、Chrome 90+, Firefox 88+, Safari 14+, Edge 90+ を推奨します。内部的にはPlaywright（Chromium）でWebページを取得しています。

Reftrix uses an MCP server + standalone CLI architecture. When displaying code generated by `layout.generate_code`, Chrome 90+, Firefox 88+, Safari 14+, Edge 90+ are recommended. Internally, Playwright (Chromium) is used to fetch web pages.
...
**A:** レスポンスの `retryAfterMs` で示されたミリ秒だけ待ってから再試行してください。レート制限はToken Bucketアルゴリズムで実装されており、analysisツールは10 RPM、searchツールは120 RPM、その他は60 RPMです。`RATE_LIMIT_ENABLED=false` で無効化も可能です。

Wait for the milliseconds indicated by `retryAfterMs` in the response, then retry. Rate limiting uses a Token Bucket algorithm: analysis tools at 10 RPM, search tools at 120 RPM, others at 60 RPM. Disable with `RATE_LIMIT_ENABLED=false`.
