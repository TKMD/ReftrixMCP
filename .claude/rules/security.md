...
- ✅ 全<!-- gen:tool-count -->40<!-- /gen:tool-count --> MCPツールに自動適用 / Auto-applied to all <!-- gen:tool-count -->40<!-- /gen:tool-count --> MCP tools
...
新規依存追加時はライセンス確認必須。
LGPL-3.0-or-later 依存は**個別にレビュー記録を残すこと**（この義務は維持する）。なお、上記 gate の `--excludePackages` が列挙するのは `reftrix@<version>` と `sharp` のみであり LGPL パッケージを一切列挙しないため、「LGPL 依存を `--excludePackages` で除外する」という従来の記述は**記述どおりには no-op** である（実測。gate は `@img/*` をそもそも検査していない）。`--onlyAllow` への `LGPL-3.0-or-later` の追加は**引き続き禁止**である。

No merge allowed with any High/Critical vulnerabilities.
License verification is mandatory when adding new dependencies.
LGPL-3.0-or-later dependencies **must be reviewed individually and the review recorded** (this obligation is retained). Note that the gate's `--excludePackages` enumerates only `reftrix@<version>` and `sharp` and no LGPL package, so the former instruction to "exclude LGPL dependencies via `--excludePackages`" is **a no-op as written** (measured: the gate never inspects `@img/*`). Adding `LGPL-3.0-or-later` to `--onlyAllow` **remains prohibited**.

## エラーメッセージサニタイズ / Error Message Sanitization

### sanitizeErrorMessage パターン / sanitizeErrorMessage Pattern

v0.2.0で `utils/sanitize-error.ts` に統一ユーティリティとして抽出。<!-- gen:sanitize-usage-count -->126<!-- /gen:sanitize-usage-count -->ファイル・<!-- gen:tool-count -->40<!-- /gen:tool-count -->ツールに適用（<!-- gen:sanitize-import-count -->113<!-- /gen:sanitize-import-count -->ファイルでインポート）。

Extracted as a unified utility in `utils/sanitize-error.ts` in v0.2.0. Applied to <!-- gen:sanitize-usage-count -->126<!-- /gen:sanitize-usage-count --> files and <!-- gen:tool-count -->40<!-- /gen:tool-count --> tools (<!-- gen:sanitize-import-count -->113<!-- /gen:sanitize-import-count --> files importing it).
...
> **Tracked: migration CLI local `truncateId(id, 8)` co-migration (FIND-IMPL-SS-L-09, L, deadline 2026-06-14 = T+1d)**: PR-SS-A (ADR-0041) の `apps/mcp-server/scripts/migrate-screenshots-to-persistent-root.ts` は local `truncateId(id, 8)` (`:94-96`) を持ち、SSOT `truncateAuditTargetId` を非経由 + `repair-truncated-screenshot-terminals.ts:105` と論理重複する (`details.targetWebPageIdTruncated` (`:365`) も同 local helper 由来)。**実害なし根拠**: length=8 は SSOT 値と一致 (現状 PII 漏洩 0)、主経路 (audit_logs targetId) は AuditLogService 境界で SSOT truncate 済、repair script precedent 整合。**defense-in-depth 候補 (将来 PR、owner backend-api-developer)**: project-wide `truncateId` SSOT 統一時に上記 migration CLI / repair script の local helper を co-migration し、§"Canonical CWE-209 PII Protection Pattern" の SSOT-derive rigor を log/details path にも適用する。
>
> **Tracked: migration CLI local `truncateId(id, 8)` co-migration (FIND-IMPL-SS-L-09, L, deadline 2026-06-14 = T+1d)**: PR-SS-A's (ADR-0041) `migrate-screenshots-to-persistent-root.ts` has a local `truncateId(id, 8)` (`:94-96`) that bypasses the SSOT `truncateAuditTargetId` and logically duplicates `repair-truncated-screenshot-terminals.ts:105` (`details.targetWebPageIdTruncated` at `:365` is from the same local helper). No current real harm because length=8 matches the SSOT value (0 PII leakage), the main path (audit_logs targetId) is SSOT-truncated at the AuditLogService boundary, and it is consistent with the repair-script precedent. Defense-in-depth candidate (future PR, owner backend-api-developer): co-migrate these local helpers during project-wide `truncateId` SSOT unification, applying the §"Canonical CWE-209 PII Protection Pattern" SSOT-derive rigor to the log/details path.
...
### Embedding worker-thread CUDA gate parity — inner-retry `console.warn` raw deviceErrorMsg (SEC-IMPL-PR1-L-01, pre-existing, L, deadline 2026-06-15 = T+1d) / Embedding worker-thread CUDA gate parity — inner-retry `console.warn` raw deviceErrorMsg（SEC-IMPL-PR1-L-01、pre-existing、L、deadline 2026-06-15 = T+1d）

**JP**: embedding fix PR-1 (CORE gate parity、IO Impl Decision = APPROVE `019ec66d`) の re-audit で SEC L finding (SEC-IMPL-PR1-L-01) として、`packages/ml/src/embeddings/worker-thread.ts:130-134` の device-fallback `console.warn` が raw `deviceErrorMsg` (= `deviceError.message`) を出力している点が確認された。これは pipeline 作成が `effectiveDevice !== "cpu"` で失敗した際の inner CPU-retry catch block 内であり、**internal worker-thread log のみ** (client 非到達) かつ **pre-existing** (PR-1 で導入された差異ではない) である。PR-1 の CORE fix (`config.device` override 撤去 → 第一試行が CUDA-EP 不在 host で cpu に解決し成功) により本 `console.warn` の到達頻度は **減少方向**。CWE-209 latent surface に留まり現リリースサイクルで実害なし → **L severity** (docs 記録 + tracked、deadline 2026-06-15 = T+1d)。

**Defense-in-depth 候補 / Defense-in-depth candidate** (将来 PR、owner security-engineer): inner-retry catch の `console.warn` を `sanitizeErrorMessage(deviceError)` 経由化し、`libonnxruntime` / `.so` / path を非開示にする。これにより `INV-EMBEDDING-WORKER-CUDA-GATE-001` の Layer (i) test が pin する CWE-209 `.so` 非開示 assert と同契約を log path にも適用する。§"Canonical CWE-209 PII Protection Pattern" の SSOT-derive rigor を error-log path にも横展開する候補。

**EN**: In the embedding fix PR-1 (CORE gate parity, IO Impl Decision = APPROVE `019ec66d`) re-audit, a SEC L finding (SEC-IMPL-PR1-L-01) confirmed that the device-fallback `console.warn` at `packages/ml/src/embeddings/worker-thread.ts:130-134` emits the raw `deviceErrorMsg` (`deviceError.message`). It is inside the inner CPU-retry catch block (entered when pipeline creation fails with `effectiveDevice !== "cpu"`), an **internal worker-thread log only** (never reaching the client), and **pre-existing** (not introduced by PR-1). PR-1's CORE fix (removing the `config.device` override so the first attempt resolves to cpu and succeeds on a CUDA-EP-absent host) **reduces** how often this `console.warn` is reached. It is a CWE-209 latent surface with no current real harm → **L severity** (docs record + tracked, deadline 2026-06-15 = T+1d). Defense-in-depth candidate (future PR, owner security-engineer): route the inner-retry `console.warn` through `sanitizeErrorMessage(deviceError)` so `libonnxruntime` / `.so` / path is non-disclosed, applying to the log path the same CWE-209 `.so`-non-disclosure contract pinned by the `INV-EMBEDDING-WORKER-CUDA-GATE-001` Layer (i) test (a candidate for cross-applying the §"Canonical CWE-209 PII Protection Pattern" SSOT-derive rigor to the error-log path).

**Cross-ref**: Finding Registry `(SEC-IMPL-PR1-L-01) /`packages/ml/src/embeddings/worker-thread.ts:130-134`(inner-retry`console.warn`raw`deviceErrorMsg`) / `packages/ml/src/embeddings/worker-thread-device.ts`(CORE gate helper, reduces reach) /`apps/mcp-server/tests/regression/standing/large-page/inv-embedding-worker-cuda-gate-001.test.ts`(Layer (i) CWE-209`.so`-non-disclosure assert) / §"Canonical CWE-209 PII Protection Pattern" (SSOT-derive rigor) / `.claude/rules/testing-requirements.md`§8 (TDA-IMPL-L-02 worker/in-process gate mirror) / IO Impl Decision = APPROVE`019ec66d` (Phase 3 docs-sync landing).

### Crop dir cascade `buildSafeCropDir` inline UUID validation defense-in-depth (W6 Issue A PR-3b, TDA-04, tracked, L, deadline 2026-06-25 = T+1d) / Crop dir cascade `buildSafeCropDir` の inline UUID 検証 defense-in-depth

**JP**: W6 Issue A PR-3b (crop GDPR cascade、IO Plan Decision V0 = CONDITIONAL `019ef9b2-3c92`) で、GDPR Art.17 crop-dir cascade (`deleteCropDirBestEffort`) が `fs.rm(cropDir, {recursive:true})` する dir を `buildSafeCropDir(webPageId)` (`crop-persistence.helper.ts:170`) で構築する。`buildSafeCropDir` は内部で `webPageId` を **`UUID_REGEX` で検証** (`assertValidCropIds` 系の inline gate) → `buildSafePathWithinRoot` 共有 core を経由して per-page dir を返す (SEC-H-02 parity、F-06)。これにより malformed id (`../sibling`, null-byte) は throw して `fs.rm` に到達しない (sibling/parent dir の再帰削除を構造的に防止)。batch path は DB-sourced `pageIds` を回すため caller 検証に依存せず **内部再検証**が必須であり、本契約は健全。

**TDA-04 finding (L、defense-in-depth、現状実害なし)**: `buildSafeCropDir` の UUID 検証は **2-引数版 `assertValidCropIds(webPageId, entityId)`** を流用しており、dir-only path (entityId 不要) でも entityId に webPageId を渡す等の **引数 shape の不一致 surface** がある (`buildSafeCropPath` は `(webPageId, kind, entityId)` の 3-arg、`buildSafeCropDir` は `(webPageId)` の 1-arg だが内部検証 helper を共有)。**現状の実害なし根拠**: 検証は `UUID_REGEX` の単一 gate であり、dir path も file path も同じ `UUID_REGEX` で webPageId を弾くため、引数 shape の不一致は検証 correctness に影響しない (両 path とも malformed webPageId を reject)。**defense-in-depth 候補** (将来 PR、owner backend-api-developer + security-engineer): `buildSafeCropDir` 専用の **単一引数 helper `assertValidWebPageId(webPageId)`** を抽出し、dir path が file-path 用の 2-arg `assertValidCropIds` を流用しない形に分離する (引数 shape の意図を code-level で明示、誤用 surface を縮退)。

**EN**: In W6 Issue A PR-3b (crop GDPR cascade, IO Plan Decision V0 = CONDITIONAL `019ef9b2-3c92`), the GDPR Art.17 crop-dir cascade (`deleteCropDirBestEffort`) `fs.rm(cropDir, {recursive:true})`s a dir built by `buildSafeCropDir(webPageId)` (`crop-persistence.helper.ts:170`). `buildSafeCropDir` internally **validates `webPageId` via `UUID_REGEX`** (an inline gate of the `assertValidCropIds` family) → returns the per-page dir through the shared `buildSafePathWithinRoot` core (SEC-H-02 parity, F-06), so a malformed id (`../sibling`, null-byte) throws before `fs.rm` (structurally preventing recursive deletion of a sibling/parent dir). The batch path iterates DB-sourced `pageIds`, so it does not depend on caller validation and **internal re-validation is mandatory** — the contract is sound. **TDA-04 finding (L, defense-in-depth, no current real harm)**: `buildSafeCropDir`'s UUID validation reuses the **2-argument `assertValidCropIds(webPageId, entityId)`**, so for a dir-only path (no entityId) there is an **argument-shape-mismatch surface** (e.g. passing webPageId as the entityId): `buildSafeCropPath` is 3-arg `(webPageId, kind, entityId)` while `buildSafeCropDir` is 1-arg `(webPageId)` yet they share the internal validation helper. No current real harm because the validation is a single `UUID_REGEX` gate and both the dir and file paths reject a malformed webPageId via the same `UUID_REGEX`, so the argument-shape mismatch does not affect validation correctness (both paths reject a malformed webPageId). Defense-in-depth candidate (future PR, owner backend-api-developer + security-engineer): extract a dedicated **single-argument helper `assertValidWebPageId(webPageId)`** for `buildSafeCropDir`, separating the dir path from reusing the file-path's 2-arg `assertValidCropIds` (making the argument-shape intent explicit at the code level and shrinking the misuse surface).

**Cross-ref**: Finding Registry `(TDA-04 / F-06 SEC-H-02 parity) /`apps/mcp-server/src/services/part/crop-persistence.helper.ts` (`buildSafeCropDir:170`/`assertValidCropIds`/`buildSafeCropPath:135`/`UUID_REGEX`SSOT import) /`apps/mcp-server/src/services/screenshot-persistence.service.ts` (`buildSafePathWithinRoot`shared core) /`apps/mcp-server/src/services/gdpr-deletion.service.ts` (`deleteCropDirBestEffort:492`) / `INV-CROP-RETENTION-001`(#6 negative-traversal throw) / §"Canonical CWE-209 PII Protection Pattern" (SSOT-derive rigor) /`.claude/rules/testing-requirements.md`§9 (TPA-02 / TDA-05 crop-retention test-infra) / IO Plan Decision V0 = CONDITIONAL`019ef9b2-3c92`.

### Crop serve route `hasCrop` part-path READ-sink belt asymmetry (W6 Issue A PR-4a, SEC-IMPL-PR4A-L-01, tracked, L, deadline 2026-06-27 = T+1d) / Crop serve route `hasCrop` part-path READ-sink belt の非対称

**JP**: W6 Issue A PR-4a (crop serve route + backfill、[ADR-0042](../specs/adr/ADR-0042-webui-internal-read-api.md) Amendment 12、IO Impl Decision V1 = APPROVE `019f028c`) の crop serve route (`getCropStream`、`page-detail.service.ts`) は serve-time PII redaction を **3 層** で構成する: (i) **structural** — high-PII crop は disk 上に存在しない (Amendment 11 §A11.1 WRITE-sink fail-closed、構造的非生成); (ii) **DB-first** — high-PII 行は `crop_storage_path` NULL → 404; (iii) **section-only belt-and-braces READ-sink** — section crop のみ `getHighPiiSectionIds` を consult して high-PII section を 404 化。**part path は layer (iii) の READ-sink belt を継承しない** (SEC-IMPL-PR4A-L-01)。

**現状の実害なし根拠 / Why no current real harm**: (1) **layer (i) structural が primary defense** — high-PII part の crop は WRITE-sink (`piiRiskLevel != "high"` filter) で構造的に非生成ゆえ disk 上に存在しない。(2) **layer (ii) DB-first NULL→404 が secondary** — high-PII part 行は `crop_storage_path` を持たないため DB-first lookup が null → 404。(3) **part は section-linked PII anchor を持たない** — `getHighPiiSectionIds` は section_pattern_id を anchor とする section-scoped helper であり、part に直接適用する自然な anchor がない (part の high-PII 判定は `component_parts.pii_risk_level` であって section helper 経由ではない)。よって part path に layer (iii) を足しても layer (i)/(ii) の NULL に対して net-0 であり、現リリースサイクルで実害はない → **L severity** (docs 記録 + tracked、deadline 2026-06-27 = T+1d)。

**Defense-in-depth 候補 / Defense-in-depth candidate** (将来 PR、owner backend-api-developer + security-engineer): part path 専用の READ-sink belt (`component_parts.pii_risk_level = 'high'` を anchor とする part-scoped guard) を `getCropStream` の part 分岐に追加し、section path の 3 層と対称化する。§"Canonical CWE-209 PII Protection Pattern" の SSOT-derive rigor を part READ-sink にも適用 (high-PII literal の hardcode 回避)。

**EN**: The crop serve route (`getCropStream`, `page-detail.service.ts`) of W6 Issue A PR-4a (crop serve route + backfill, [ADR-0042](../specs/adr/ADR-0042-webui-internal-read-api.md) Amendment 12, IO Impl Decision V1 = APPROVE `019f028c`) composes serve-time PII redaction in **3 layers**: (i) **structural** (a high-PII crop is never on disk — Amendment 11 §A11.1 WRITE-sink fail-closed), (ii) **DB-first** (a high-PII row has `crop_storage_path` NULL → 404), and (iii) a **section-only belt-and-braces READ-sink** (only the section crop consults `getHighPiiSectionIds` to 404 a high-PII section). **The part path does NOT inherit the layer (iii) READ-sink belt** (SEC-IMPL-PR4A-L-01). No current real harm because (1) layer (i) structural is the primary defense — a high-PII part's crop is structurally non-generated by the WRITE-sink (`piiRiskLevel != "high"` filter), so it is never on disk; (2) layer (ii) DB-first NULL→404 is secondary — a high-PII part row carries no `crop_storage_path`, so the DB-first lookup returns null → 404; and (3) a part has no section-linked PII anchor — `getHighPiiSectionIds` is a section-scoped helper anchored on `section_pattern_id`, with no natural anchor to apply directly to a part (a part's high-PII determination is `component_parts.pii_risk_level`, not via the section helper). So adding layer (iii) to the part path is net-0 against the layer (i)/(ii) NULL, with no real harm in the current release cycle → **L severity** (docs record + tracked, deadline 2026-06-27 = T+1d). Defense-in-depth candidate (future PR, owner backend-api-developer + security-engineer): add a part-specific READ-sink belt (a part-scoped guard anchored on `component_parts.pii_risk_level = 'high'`) to the `getCropStream` part branch, symmetrising it with the section path's 3 layers; apply the §"Canonical CWE-209 PII Protection Pattern" SSOT-derive rigor to the part READ-sink too (avoiding a hardcoded high-PII literal).

**Cross-ref**: Finding Registry `(SEC-IMPL-PR4A-L-01 / F-L-01 part READ-sink asymmetry) /`apps/mcp-server/src/api/internal/page-detail.service.ts` (`getCropStream`part vs section branch /`getHighPiiSectionIds`section-only READ-sink) / [ADR-0042](../specs/adr/ADR-0042-webui-internal-read-api.md) §Amendment 12 §A12.1 (3-layer serve-time PII redaction) + §Amendment 11 §A11.1 (WRITE-sink fail-closed) /`INV-CROP-SERVE-PII-REDACTION-001`(in`tests/api/internal/crop-route.test.ts`) / §"Canonical CWE-209 PII Protection Pattern" (SSOT-derive rigor) / IO Impl Decision V1 = APPROVE `019f028c`.

### `crop_backfill_run` bare literal vs SSOT audit-action (W6 Issue A PR-4a, SEC-L-02 = LCC-L-01 = TDA-L-02 convergent, tracked, L, deadline 2026-06-27 = T+1d) / `crop_backfill_run` bare literal vs SSOT audit-action（convergent）

**JP**: W6 Issue A PR-4a の one-shot operator script `apps/mcp-server/scripts/backfill-crops.ts` は per-run Art.30 audit を `action: "crop_backfill_run"` (`:411`) の **bare string literal** で emit する。一方、robots-disallow skip の audit action `crop_backfill_robots_disallowed` は SSOT 定数 `AUDIT_ACTION_CROP_BACKFILL_ROBOTS_DISALLOWED` (`audit-actions.ts`、`:77` で import) を経由する。本 finding は SEC / LCC / TDA の 3 監査が独立に同一指摘へ収束した convergent L (SEC-L-02 = LCC-L-01 = TDA-L-02): `crop_backfill_run` を SSOT 定数化していない命名の非対称。

**現状の実害なし根拠 / Why no current real harm**: (1) **`scripts/` は `INV-AUDIT-EMIT-SSOT-IMPORT-001` の production sweep 対象外** — 当該 standing test の AST sweep は production `src/**` を scope とし (`scripts/repair-false-failed-backfill.ts` の様な src 配下 helper は対象だが、top-level `apps/mcp-server/scripts/` operator script は scope 外)、bare literal を CI で fail させない。(2) **actor は SSOT 済** — `AUDIT_ACTOR_CROP_BACKFILL` (`system:crop-backfill`) は SSOT 定数経由 (§"Worker actor naming SSOT" 準拠)、`details` は PII-free numeric/enum + `truncateAuditTargetId` で CWE-209 clean。(3) **robots action は既に SSOT** — 同 script 内の隣接 audit emit (`crop_backfill_robots_disallowed`) は SSOT 定数を使うため、`crop_backfill_run` のみが命名規律の outlier。よって GDPR Art.30 record-keeping は機能し (action 名は安定リテラル)、coupling-drift latent surface に留まり現リリースサイクルで実害はない → **L severity** (docs 記録 + tracked、deadline 2026-06-27 = T+1d)。

**Defense-in-depth 候補 / Defense-in-depth candidate** (将来 PR、owner backend-api-developer): `crop_backfill_run` を SSOT 定数 `AUDIT_ACTION_CROP_BACKFILL_RUN` (`audit-actions.ts`) 化し、`backfill-crops.ts:411` が import 経由で emit するよう統一する (§"Worker actor naming SSOT" の SSOT-derive rigor を audit-action にも適用、bare literal hardcode を排除)。`INV-AUDIT-EMIT-SSOT-IMPORT-001` の AST sweep scope を top-level `scripts/` operator script にも拡張する候補と連関 (sweep が拡張されれば coupling-drift が CI 検出可能化)。

**EN**: The one-shot operator script `apps/mcp-server/scripts/backfill-crops.ts` of W6 Issue A PR-4a emits the per-run Art.30 audit with a **bare string literal** `action: "crop_backfill_run"` (`:411`), whereas the robots-disallow skip audit action `crop_backfill_robots_disallowed` routes through the SSOT constant `AUDIT_ACTION_CROP_BACKFILL_ROBOTS_DISALLOWED` (`audit-actions.ts`, imported at `:77`). This finding is a convergent L where the SEC / LCC / TDA audits independently converged on the same point (SEC-L-02 = LCC-L-01 = TDA-L-02): the naming asymmetry of not SSOT-constant-izing `crop_backfill_run`. No current real harm because (1) **`scripts/` is outside the `INV-AUDIT-EMIT-SSOT-IMPORT-001` production sweep** — that standing test's AST sweep scopes the production `src/**` (an under-src helper like `scripts/repair-false-failed-backfill.ts` is in scope, but a top-level `apps/mcp-server/scripts/` operator script is out of scope), so the bare literal is not CI-failed; (2) **the actor is SSOT-ized** — `AUDIT_ACTOR_CROP_BACKFILL` (`system:crop-backfill`) routes through the SSOT constant (per §"Worker actor naming SSOT"), and `details` are PII-free numeric/enum + `truncateAuditTargetId` (CWE-209 clean); and (3) **the robots action is already SSOT** — the adjacent audit emit (`crop_backfill_robots_disallowed`) in the same script uses the SSOT constant, so only `crop_backfill_run` is the naming-discipline outlier. Hence GDPR Art.30 record-keeping functions (the action name is a stable literal), this is a coupling-drift latent surface only, and there is no real harm in the current release cycle → **L severity** (docs record + tracked, deadline 2026-06-27 = T+1d). Defense-in-depth candidate (future PR, owner backend-api-developer): SSOT-constant-ize `crop_backfill_run` as `AUDIT_ACTION_CROP_BACKFILL_RUN` (`audit-actions.ts`) and unify `backfill-crops.ts:411` to emit via an import (applying the §"Worker actor naming SSOT" SSOT-derive rigor to the audit-action too, eliminating the bare-literal hardcode); coupled with a candidate to extend the `INV-AUDIT-EMIT-SSOT-IMPORT-001` AST sweep scope to top-level `scripts/` operator scripts (so that coupling-drift becomes CI-detectable once the sweep is extended).

**Cross-ref**: Finding Registry `(SEC-L-02 = LCC-L-01 = TDA-L-02 convergent) /`apps/mcp-server/scripts/backfill-crops.ts` (`crop_backfill_run`bare literal`:411`vs`AUDIT_ACTION_CROP_BACKFILL_ROBOTS_DISALLOWED`SSOT import`:77`) / `apps/mcp-server/src/audit/audit-actions.ts` (`AUDIT_ACTION_CROP_BACKFILL_ROBOTS_DISALLOWED`+`AUDIT_ACTOR_CROP_BACKFILL`SSOT) /`apps/mcp-server/tests/regression/standing/gdpr-delete/inv-audit-emit-ssot-import-001.test.ts`(production`src/\*\*`sweep scope) / §"Worker actor naming SSOT" (SSOT-derive rigor) / IO Impl Decision V1 = APPROVE`019f028c`.

### webui crop proxy `UUID_EARLY_REJECT` inline literal vs mcp-server SSOT validator (W6 Issue A PR-4b, SEC-IMPL-PR4B-L-01 = TDA-IMPL-PR4B-L-02 convergent, tracked, L, deadline 2026-06-27 = T+1d) / webui crop proxy `UUID_EARLY_REJECT` の inline literal vs mcp-server SSOT validator（convergent）

**JP**: W6 Issue A PR-4b の同一オリジン crop proxy route `apps/webui/src/app/api/crop/[kind]/[entityId]/[pageId]/route.ts` は、cheap な same-origin early-reject 用に **inline UUID regex literal `UUID_EARLY_REJECT`** を持つ (kind/entityId/pageId を 400 で早期 reject)。webui は mcp-server の SSOT validator (`screenshot-persistence.service.ts` の `UUID_REGEX` / 内部 API の `cropParamsSchema`) を **package boundary 越しに import 不可** (webui は `apps/mcp-server/**` の export を直接参照できない — `INV-WEBUI-READONLY-NEGATIVE-001` の layer-boundary 契約)。本 finding は SEC / TDA の 2 監査が独立に同一指摘へ収束した convergent L (SEC-IMPL-PR4B-L-01 = TDA-IMPL-PR4B-L-02): authoritative validation を SSOT に集約していない inline literal の存在。

**現状の実害なし根拠 / Why no current real harm**: (1) **authoritative validation は 24006 server-side** — proxy route の `UUID_EARLY_REJECT` は **defense-in-depth であり gate ではない**。実 validation は 24006 の `cropParamsSchema` (UUID×2 + `kind` enum) + `validateCropPath` realpath path-traversal + high-PII fail-closed (3 層 redaction) が担う。webui inline regex が緩く/誤って通過させても、24006 が malformed/high-PII を確実に reject (400/404) し crop bytes を返さない。(2) **screenshot route precedent と整合** — 既存 W2 screenshot proxy `app/api/screenshot/[id]/route.ts` も同じ理由で inline `UUID_EARLY_REJECT` を持つ (webui は同 SSOT を import 不可、authoritative @ 24006)。すなわち PR-4b は既存の確立済 pattern を踏襲しており新規 surface ではない。(3) **inline literal は behavior-correct** — canonical UUID (v1-v8) pattern を正しく early-reject する (JSDoc に「AUTHORITATIVE validation lives server-side at 24006」と honest 明記)。よって CWE-20 (improper input validation) の latent surface に留まり現リリースサイクルで実害はない → **L severity** (docs 記録 + tracked、deadline 2026-06-27 = T+1d)。

**Defense-in-depth 候補 / Defense-in-depth candidate** (将来 PR、owner frontend-engineer + security-engineer): webui-local の UUID regex を webui 内の **共有 const** (例: `apps/webui/src/lib/validation/uuid.ts` の `WEBUI_UUID_REGEX`) に集約し、crop proxy route と screenshot proxy route + (将来の) compare route が同一 webui-local SSOT を import するよう統一する。これにより webui-local の inline UUID literal 重複 (現状 crop + screenshot の 2 箇所) を webui 境界内で SSOT 化し、mcp-server の SSOT validator (package boundary 越しに import 不可) とは独立に webui-local の coupling-drift を防止する。authoritative validation は 24006 に残す (webui SSOT は defense-in-depth pre-pin の純化)。

**EN**: The same-origin crop proxy route `apps/webui/src/app/api/crop/[kind]/[entityId]/[pageId]/route.ts` of W6 Issue A PR-4b carries an **inline UUID regex literal `UUID_EARLY_REJECT`** for a cheap same-origin early-reject (kind/entityId/pageId rejected with 400). The webui **cannot import the mcp-server SSOT validator** (`UUID_REGEX` in `screenshot-persistence.service.ts` / `cropParamsSchema` in the internal API) across the package boundary (the webui may not directly reference `apps/mcp-server/**` exports — the `INV-WEBUI-READONLY-NEGATIVE-001` layer-boundary contract). This finding is a convergent L where the SEC / TDA audits independently converged on the same point (SEC-IMPL-PR4B-L-01 = TDA-IMPL-PR4B-L-02): the existence of an inline literal not consolidated into an SSOT for the authoritative validation. No current real harm because (1) **the authoritative validation is server-side at 24006** — the proxy route's `UUID_EARLY_REJECT` is **defense-in-depth, not the gate**; the real validation is the 24006 `cropParamsSchema` (UUID×2 + `kind` enum) + `validateCropPath` realpath path-traversal + high-PII fail-closed (3-layer redaction), so even if the webui inline regex loosely / wrongly let something through, the 24006 reliably rejects malformed / high-PII (400/404) and returns no crop bytes; (2) **it is consistent with the screenshot-route precedent** — the existing W2 screenshot proxy `app/api/screenshot/[id]/route.ts` carries the same inline `UUID_EARLY_REJECT` for the same reason (the webui cannot import the same SSOT; authoritative @ 24006), so PR-4b follows an established pattern rather than introducing a new surface; and (3) the inline literal is behavior-correct — it correctly early-rejects against the canonical UUID (v1-v8) pattern (the JSDoc honestly states "the AUTHORITATIVE validation lives server-side at 24006"). Hence this is a CWE-20 (improper input validation) latent surface only, with no real harm in the current release cycle → **L severity** (docs record + tracked, deadline 2026-06-27 = T+1d). Defense-in-depth candidate (future PR, owner frontend-engineer + security-engineer): consolidate the webui-local UUID regex into a **shared const inside the webui** (e.g. `WEBUI_UUID_REGEX` in `apps/webui/src/lib/validation/uuid.ts`) so the crop proxy route and the screenshot proxy route + (a future) compare route import the same webui-local SSOT; this SSOT-izes the webui-local inline UUID-literal duplication (currently the 2 sites crop + screenshot) within the webui boundary and prevents webui-local coupling-drift independently of the mcp-server SSOT validator (not importable across the package boundary), while leaving the authoritative validation at 24006 (the webui SSOT purifies the defense-in-depth pre-pin only).

**Cross-ref**: Finding Registry `(SEC-IMPL-PR4B-L-01 = TDA-IMPL-PR4B-L-02 convergent) /`apps/webui/src/app/api/crop/[kind]/[entityId]/[pageId]/route.ts` (`UUID_EARLY_REJECT`inline literal, defense-in-depth) /`apps/webui/src/app/api/screenshot/[id]/route.ts`(screenshot-route`UUID_EARLY_REJECT`precedent) /`apps/mcp-server/src/api/internal/schemas.ts` (`cropParamsSchema`authoritative @ 24006) /`apps/mcp-server/src/services/screenshot-persistence.service.ts` (`UUID_REGEX`SSOT, not importable across the package boundary) /`apps/webui/tests/security/readonly-layer-boundary.test.ts` (`INV-WEBUI-CROP-READONLY`layer boundary) / IO Impl Decision V0 = APPROVE`019f02fc`.

**Status update (W7b-0, IO Impl Decision = APPROVE `019f2b92-7df1`): SEC-IMPL-PR4B-L-01 (= TDA-IMPL-PR4B-L-02 convergent) → FIXED**. 本ファイル §"L-tracked defense-in-depth bundle PR — tracked-issue status update" の supersede 方式に従い、本 update は上記 origin prose の **status を確定的に supersede** する (origin prose は origin record として保持、status のみ本 update が canonical)。

**JP**: W7b-0 (webui-local UUID/handle SSOT chore PR) が documented defense-in-depth candidate どおり、webui-local UUID regex を `apps/webui/src/lib/validation/uuid.ts` の `WEBUI_UUID_REGEX` に SSOT 集約した。crop / screenshot / compare の **3 site** が同 SSOT を import し、**inline UUID literal 残存 0**。`INV-WEBUI-CROP-TILE-HANDLE-SSOT` (`apps/webui/tests/security/crop-tile-handle-ssot.test.ts`) が inline 残存 0 を pin。`WEBUI_UUID_REGEX` は退役した inline literal と **byte-identical** ゆえ early-reject 防御は非弱体化、authoritative validation は依然 24006 (`cropParamsSchema` + `validateCropPath` + high-PII fail-closed) のまま (webui SSOT は defense-in-depth pre-pin の純化のみ)。SEC 発行元 sign-off = **GRANTED**。
...
### `sanitizeErrorMessage` on server-side logs masks root cause — sweep-catch asymmetry hid a HIGH `__name` total loss (W6 Issue A `__name` fix, SEC-L-01, tracked, L, deadline 2026-07-02 = T+1d) / server-side ログへの `sanitizeErrorMessage` 適用が root cause を隠蔽 — sweep-catch の非対称が HIGH の `__name` 全損を隠した

**JP**: `sanitizeErrorMessage` は **client-facing response 用** の sanitiser であり、**server-side log に適用してはならない**。`apps/mcp-server/src/utils/sanitize-error.ts:59` の docstring 自身が「Server-side logging should use the original error object」と明記し、:68 で `logger.error("Operation failed", { error });`（生 error object）を exemplar として示している。W6 Issue A の `__name` bug ではこの規約が破られていた: `part-bbox-playwright.service.ts` の `runBboxScrollSweep` sweep catch（~L968-977）が `page.evaluate` 例外を `error: sanitizeErrorMessage(sweepEvalError)`（~L975）で記録していたため、browser-context の `ReferenceError: __name is not defined`（毎ページ `resolvedCount: 0` = 深部 part bbox 解決の HIGH 全損）が `logger.warn` 上で「An internal error occurred」相当の generic message に潰され、根本原因が長期間 observability から隠蔽されていた。同 file の sibling catch（~L608 の `error: error instanceof Error ? error.message : String(error)`）は生 `error.message` を記録しており、sweep catch だけが非対称に sanitise していた = この非対称が masking を生んだ。

**現状の扱い / Current status**: `__name` の真因は本 PR の fix（argument-position 分解）+ Tier1 INV（`INV-PAGE-EVALUATE-NO-NAME-INJECTION-001`）+ DoD「warn 消失」で **当該 failure mode はカバー済**。ただし masking 機構（server-side log への sanitiser 適用）自体は moot ではなく、別の server-side throw を将来同様に隠蔽しうる latent surface に留まる → **L severity**（docs 記録 + tracked、deadline 2026-07-02 = T+1d）。

**Defense-in-depth 候補 / Defense-in-depth candidate**（将来 PR、owner capture-embedding-engineer + security-engineer）: server-side catch（特に `runBboxScrollSweep` sweep catch）では生 `error.message` を log する（または raw + sanitized を併記する）。browser-context の `ReferenceError` 等は **PII ではない**ため、server log への raw 出力は CWE-209 client-exposure に該当しない（client response にだけ `sanitizeErrorMessage` を適用する原則は不変）。`isDevelopment()` ガードは使わない（全環境で warn/error を出す、`.claude/rules/coding-standards.md` §isDevelopment() ガード原則）。これにより sibling catch（生 `error.message`）と sweep catch の log 規約を対称化し、HIGH-severity な server-side throw の root-cause observability を回復する。

**EN**: `sanitizeErrorMessage` is a **client-facing-response** sanitiser and **MUST NOT be applied to server-side logs**. The docstring of `apps/mcp-server/src/utils/sanitize-error.ts:59` itself states "Server-side logging should use the original error object" and shows `logger.error("Operation failed", { error });` (the raw error object) as the exemplar at :68. The W6 Issue A `__name` bug violated this: the `runBboxScrollSweep` sweep catch (~L968-977) in `part-bbox-playwright.service.ts` recorded the `page.evaluate` exception via `error: sanitizeErrorMessage(sweepEvalError)` (~L975), so the browser-context `ReferenceError: __name is not defined` (a HIGH total loss — `resolvedCount: 0` on every page = all deep-part bbox resolution lost) was collapsed on `logger.warn` into a generic "An internal error occurred"-equivalent message, hiding the root cause from observability for a long time. A sibling catch in the same file (~L608: `error: error instanceof Error ? error.message : String(error)`) logs the raw `error.message`, so only the sweep catch sanitised asymmetrically — that asymmetry is what produced the masking. **Current status**: the `__name` root cause is now **covered** by this PR's fix (argument-position decomposition) + the Tier1 INV (`INV-PAGE-EVALUATE-NO-NAME-INJECTION-001`) + the DoD "warn disappeared", but the masking mechanism itself (applying the client sanitiser to a server-side log) is not moot — it is a latent surface that could similarly hide a future server-side throw → **L severity** (docs record + tracked, deadline 2026-07-02 = T+1d). Defense-in-depth candidate (future PR, owner capture-embedding-engineer + security-engineer): in the server-side catch (notably the `runBboxScrollSweep` sweep catch) log the raw `error.message` (or raw + sanitized side by side); a browser-context `ReferenceError` is **NOT PII**, so a raw server-log line is not a CWE-209 client exposure (the rule that `sanitizeErrorMessage` applies only to the client response is unchanged), and do NOT use an `isDevelopment()` guard (emit warn/error in all environments, per `.claude/rules/coding-standards.md` §isDevelopment() guard principle). This symmetrises the log discipline between the sibling catch (raw `error.message`) and the sweep catch and restores root-cause observability for HIGH-severity server-side throws.

**Cross-ref**: Finding Registry `(SEC-L-01 = L-1; IO Impl Decision = APPROVE) /`apps/mcp-server/src/utils/sanitize-error.ts:52-68`(client-only sanitiser; "Server-side logging should use the original error object") /`apps/mcp-server/src/services/part/part-bbox-playwright.service.ts` (`runBboxScrollSweep`sweep catch ~L975`sanitizeErrorMessage(sweepEvalError)`vs sibling raw`error.message`catch ~L608) /`INV-PAGE-EVALUATE-NO-NAME-INJECTION-001`(the`\_\_name`failure mode covered) / §"エラーメッセージサニタイズ / Error Message Sanitization" (client-facing sanitise policy) /`.claude/rules/coding-standards.md` §isDevelopment() ガード原則 (no catch-block isDevelopment guard).

### postcss ベース CSS parser の再導入 gate — `CSSAnimationParser` 削除後の残存記録 (`CO-SECDEPS-03`, PR-S2 で記録 / CLOSED, deadline 2026-09-04 = T+1d) / Re-introduction gate for a postcss-based CSS parser — residual record after the `CSSAnimationParser` deletion (`CO-SECDEPS-03`, recorded by PR-S2 / CLOSED)

**JP**: `packages/webdesign-core` の CSS animation parser (`CSSAnimationParser`、`postcss` ベース) は **PR-S2 で削除済み** (deleted: `packages/webdesign-core/src/motion-detector/css-animation-parser.ts` および `packages/webdesign-core/tests/motion-detector/css-animation-parser.test.ts`)。削除時点で当該 class の production 到達経路は **0** であった (`packages/webdesign-core` の `exports` は `"."` のみで subpath を持たず、同 package の barrel は当該 class を re-export しない)。稼働している同名の別実装 `apps/mcp-server/src/services/page/css-animation-parser.service.ts` は regex ベースであり `postcss` に依存しない。

**契約 / Contract**: **将来、crawled された第三者 CSS を `postcss`（または同等の full CSS parser）に食わせる parser を再導入する場合は、その再導入 PR で SEC 監査を必須とする。** 削除によって「攻撃者制御でありうる第三者 CSS が full CSS parser に到達する」入力経路が閉じているため、再導入は当該経路の復活に相当する。なお `postcss` ノード自体は override を維持したまま依存グラフに残存しており (`@tailwindcss/postcss` / `next` / `vite` の 3 edge)、削除は advisory を 1 件も閉じない — advisory を閉じたのは override floor の引上げ (`>=8.5.10` → `>=8.5.18`、解決版 `postcss@8.5.27`) である。したがって本 gate が守るのは「バージョン契約」ではなく「入力経路の再開」である。

**EN**: The `packages/webdesign-core` CSS animation parser (`CSSAnimationParser`, postcss-based) was **deleted in PR-S2** together with its test. At the time of deletion the class had **0** production reach paths (the package's `exports` map is `"."` only with no subpath, and the package barrel does not re-export the class), and the live same-named implementation `apps/mcp-server/src/services/page/css-animation-parser.service.ts` is regex-based and does not depend on `postcss`. **Contract**: if a parser that feeds crawled third-party CSS into `postcss` (or an equivalent full CSS parser) is ever re-introduced, a **SEC audit is mandatory in the re-introducing PR** — the deletion closed the path by which potentially attacker-controlled third-party CSS reaches a full CSS parser, so a re-introduction is a re-opening of that path. Note that the `postcss` node itself remains in the dependency graph behind the retained override (3 edges: `@tailwindcss/postcss` / `next` / `vite`), and the deletion closes **0** advisories — the advisories were closed by raising the override floor (`>=8.5.10` → `>=8.5.18`, resolving `postcss@8.5.27`). What this gate guards is therefore the re-opening of an input path, not a version contract.
