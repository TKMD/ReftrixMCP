...
const WORKER_THREAD_TYPES_PATH = path.resolve(
  __dirname_local,
  "../src/embeddings/worker-thread-types.ts"
);
...
/** package.json */
const PACKAGE_JSON_PATH = path.resolve(__dirname_local, "../package.json");

/** docker-compose.yml */
const DOCKER_COMPOSE_PATH = path.resolve(__dirname_local, "../../../docker/docker-compose.yml");

/** .env.example (root) */
const ROOT_ENV_EXAMPLE_PATH = path.resolve(__dirname_local, "../../../.env.example");

/** .env.example (mcp-server) */
const MCP_ENV_EXAMPLE_PATH = path.resolve(__dirname_local, "../../../apps/mcp-server/.env.example");
...
function extractNumericConstant(source: string, name: string): number | null {
  ...
  const pattern = new RegExp(`(?:const|let|var)\\s+${name}\\s*=\\s*([\\d_]+)`);
...
describe("P2-G: ONNX Worker Thread化", () => {
  ...
  describe("crash recovery（最大5回自動再起動）", () => {
    ...
    it("max restarts超過時にエラーをスローすること", () => {
      expect(serviceSource).toContain("Worker thread exceeded max restarts");
    });
...
describe("v0.1.0 Worker Thread Architecture", () => {
  ...
  it("Worker Threadのconfig.pipelineRecycleThresholdがServiceのDEFAULT値と一致すること", () => {
    ...
    const serviceDefault = extractNumericConstant(
      serviceSource,
  ...
  it("in-processバッチ推論にsetImmediateのyield pointがあること", () => {
    ...
    const batchMethod = serviceSource.slice(
      methodStart,
      methodEnd > 0 ? methodEnd : methodStart + 2000
    );
