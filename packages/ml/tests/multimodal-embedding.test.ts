...
Rhythm: regular visual rhythm with consistent spacing.`;
    ...
    it("日本語テキストでも動作", async () => {
      const japaneseInput: MultimodalEmbeddingInput = {
        textRepresentation: "レイアウト: 2カラムグリッド、ヘッダーとサイドバー付き",
  ...
  describe("createMultimodalEmbedding", () => {
    describe("text_only モード", () => {
      ...
      it("text_onlyモードではvisionFeaturesが無視される", async () => {
        ...
        const result = await service.createMultimodalEmbedding("Test content", visionFeatures, {
          searchMode: "text_only",
        });
      ...
      it("text_onlyモードでcombinedEmbeddingがtextEmbeddingと同等", async () => {
        const result = await service.createMultimodalEmbedding("Test content", undefined, {
          searchMode: "text_only",
        });
    ...
    describe("vision_only モード", () => {
      it("visionFeaturesのみでvision_onlyモードで動作する", async () => {
        const visionFeatures = createSampleVisionFeatures();
        const result = await service.createMultimodalEmbedding("Ignored content", visionFeatures, {
      ...
      it("vision_onlyモードでvisionFeaturesがない場合はエラー", async () => {
        await expect(
          service.createMultimodalEmbedding("Test content", undefined, {
            searchMode: "vision_only",
          })
        ).rejects.toThrow();
      });
    ...
    describe("metadata", () => {
      it("metadataにtextRepresentationが含まれる", async () => {
        const result = await service.createMultimodalEmbedding("Hero section design", undefined, {
          searchMode: "text_only",
        });
    ...
    describe("L2正規化検証", () => {
      ...
      it("visionEmbeddingがL2正規化されている", async () => {
        const visionFeatures = createSampleVisionFeatures();
        const result = await service.createMultimodalEmbedding("Ignored", visionFeatures, {
          searchMode: "vision_only",
        });
  ...
  describe("パフォーマンス目標", () => {
    it("単一Embedding生成が300ms以内（モック）", async () => {
      ...
      await service.createMultimodalEmbedding("Test content", visionFeatures, {
        searchMode: "combined",
      });
