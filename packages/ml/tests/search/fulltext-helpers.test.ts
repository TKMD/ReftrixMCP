...
describe("buildFulltextRankExpression", () => {
  ...
  it("デフォルトパラメータで正しいts_rank_cd式を生成すること", () => {
    ...
    // Assert
    expect(result).toBe("ts_rank_cd(se.search_vector, plainto_tsquery('english', $1))");
  });
  ...
  it("パラメータインデックス $5 で正しく生成すること", () => {
    ...
    // Assert
    expect(result).toBe("ts_rank_cd(se.search_vector, plainto_tsquery('english', $5))");
  });
  ...
  it("カラム名 'bde.search_vector' で正しく生成すること", () => {
    ...
    // Assert
    expect(result).toBe("ts_rank_cd(bde.search_vector, plainto_tsquery('english', $1))");
  });
  ...
  it("カラム名 'ne.search_vector' で正しく生成すること", () => {
    ...
    // Assert
    expect(result).toBe("ts_rank_cd(ne.search_vector, plainto_tsquery('english', $3))");
  });
