...
describe("validateDownloadUrl", () => {
  ...
  it("should reject non-HTTPS URLs", () => {
    expect(() =>
      validateDownloadUrl("http://huggingface.co/Xenova/dinov2-base/resolve/main/onnx/model.onnx")
    ).toThrow("Download URL must use HTTPS");
  });

  it("should reject non-allowlisted hosts", () => {
    expect(() => validateDownloadUrl("https://evil.com/model.onnx")).toThrow(
  ...
  it("should reject github.com", () => {
    expect(() =>
      validateDownloadUrl("https://github.com/facebookresearch/dinov2/raw/main/model.onnx")
    ).toThrow("Download host not in allowlist");
  });

  it("should reject URLs with subdomain spoofing", () => {
    expect(() => validateDownloadUrl("https://huggingface.co.evil.com/model.onnx")).toThrow(
      "Download host not in allowlist"
    );
  });

  it("should reject FTP protocol", () => {
    expect(() => validateDownloadUrl("ftp://huggingface.co/model.onnx")).toThrow(
...
describe("validateModelPath", () => {
  ...
  it("should reject paths with .. traversal", () => {
    expect(() => validateModelPath("../../../etc/passwd")).toThrow("Path traversal detected");
  });

  it("should reject paths with embedded .. segments", () => {
    expect(() => validateModelPath("packages/ml/../../../etc/shadow")).toThrow(
      "Path traversal detected"
    );
  });

  it("should reject double-dot in middle of path", () => {
    expect(() => validateModelPath("models/../../../tmp/exploit")).toThrow(
      "Path traversal detected"
    );
  });
...
describe("computeFileHash", () => {
  ...
  it("should reject non-existent file", async () => {
    await expect(computeFileHash("/tmp/nonexistent-dinov2-file.onnx")).rejects.toThrow();
  });
});
...
describe("validateFileSize", () => {
  ...
  it("should reject a file exceeding the size cap", () => {
    ...
    expect(() => validateFileSize(filePath)).toThrow("Model file exceeds size cap");
  });

  it("should reject non-existent file", () => {
    expect(() => validateFileSize(path.join(testDir, "nonexistent.bin"))).toThrow();
  });
});
