 ...
 * @returns ソース固有の説明
 */
function generateSourceSpecificDescription(
  source: string,
  keywords: string[],
  purpose?: string | null
): string {
...
export function buildDescriptionParts(metadata: AssetMetadata): DescriptionParts {
  ...
  // メイン説明を生成
  const mainDescription = generateSourceSpecificDescription(
    parsed.source,
    parsed.keywords,
    purpose
  );
  ...
  if (parsed.source === "openpeeps") {
    ...
    styleInfo = STYLE_TRANSLATIONS[style]
      ? `${STYLE_TRANSLATIONS[style]}スタイル`
      : `${style}スタイル`;
  }
...
export function generateDescription(
  ...
  const { maxLength = 200, includeSourceInfo = true, includeUsageHint = true } = options;
