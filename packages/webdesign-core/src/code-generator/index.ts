  ...
  private generateNextJS(
    ...
    switch (this.options.styling) {
      case "css-modules":
        imports.push(`import styles from './${componentName}.module.css'`);
        styles = this.generateCSSModules(section, componentName, inspectResult);
        componentCode = this.generateNextJSCSSModulesComponent(
          section,
          componentName,
          hasInteractivity,
          inspectResult
      ...
      case "styled-components":
        imports.push("import styled from 'styled-components'");
        dependencies.push("styled-components");
        componentCode = this.generateNextJSStyledComponent(
          section,
          componentName,
          hasInteractivity,
          inspectResult
      ...
      case "vanilla":
        styles = this.generateVanillaCSS(section, componentName, inspectResult);
        componentCode = this.generateNextJSVanillaComponent(
          section,
          componentName,
          hasInteractivity,
          inspectResult
      ...
      case "tailwind":
      default:
        componentCode = this.generateNextJSTailwindComponent(
          section,
          componentName,
          hasInteractivity,
          inspectResult
...
${jsx}
  ...
  private generateHeroJSX(
    ...
    const bgClass = style.hasGradient
      ? "bg-gradient-to-r from-indigo-600 to-purple-600"
      : "bg-gray-900";
    const maxWidth = inspectResult?.grid?.maxWidth
      ? `max-w-[${inspectResult.grid.maxWidth}px]`
      : "max-w-7xl";
    ...
    const responsiveText = this.options.responsive
  ...
  private generateFeatureJSX(
    ...
    const gridCols = this.options.responsive
      ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"
      : "grid-cols-3";

    const features = subHeadings
      .map((heading, index) => {
  ...
  private generateFooterJSX(
    ...
    const links = content.links;
    const copyright = content.paragraphs.find(
      (p) => p.includes("2024") || p.includes("(C)") || p.includes("copyright")
    );
          ...
          <div>
            ...
            <ul className="space-y-2">
              ${links
                ...
                .map(
                  (link) =>
                    `<li><a href="${escapeText(link.href)}" className="hover:text-white transition">${escapeJsxText(link.text)}</a></li>`
                )
                .join("\n              ")}
            </ul>
          </div>`
  ...
  private generateTestimonialJSX(
    section: DetectedSection,
    _styling: string,
    _inspectResult?: LayoutInspectOutput
  ): string {
...
${jsx}
  ...
  private generateNextJSNavigationJSX(
    section: DetectedSection,
    _inspectResult?: LayoutInspectOutput
  ): string {
    const { content } = section;
...
${jsx}
  ...
  private generateNextJSVanillaComponent(
    ...
    return (
      useClientDirective + this.generateVanillaReactComponent(section, componentName, inspectResult)
    );
  }
      ...
      <div class="${toBemClass(blockName, "column")}">
        ...
        <ul class="${toBemClass(blockName, "column-links")}">
          ${links
            .slice(idx * 2, idx * 2 + 2)
            .map(
              (link) => `<li><a href="${escapeText(link.href)}">${escapeText(link.text)}</a></li>`
            )
            .join("\n          ")}
        </ul>
      </div>`
  ...
  private generateTestimonialHTML(section: DetectedSection, blockName: string): string {
    ...
    <div class="${toBemClass(blockName, "grid")}">
      ...
      <div class="${toBemClass(blockName, "item")}">
        ...
        ${
          avatar
            ? `<div class="${toBemClass(blockName, "author")}">
