...
import { TemplateEngine, Template, TemplateContext } from "../../src/template-engine";
  ...
  describe("if条件分岐", () => {
    ...
    it("複数のif条件を処理できる", () => {
      const template: Template = {
        ...
        sectionType: "hero",
        framework: "react",
        content: "{{#if title}}<h1>{{title}}</h1>{{/if}}{{#if subtitle}}<p>{{subtitle}}</p>{{/if}}",
      };
  ...
  describe("each ループ", () => {
    ...
    it("ネストしたプロパティにアクセスできる", () => {
      const template: Template = {
        ...
        sectionType: "feature",
        framework: "react",
        content: "<ul>{{#each users}}<li>{{profile.age}}</li>{{/each}}</ul>",
      };
      ...
      const context: TemplateContext = {
        section: { type: "feature" } as any,
        options: { framework: "react" } as any,
        users: [{ profile: { age: 25 } }, { profile: { age: 30 } }, { profile: { age: 35 } }],
      };
    ...
    it("ループ内でインデックスを使用できる", () => {
      const template: Template = {
        ...
        sectionType: "feature",
        framework: "react",
        content: "<ul>{{#each items}}<li>{{@index}}: {{this}}</li>{{/each}}</ul>",
      };
      ...
      const result = engine.render(template, context);
      expect(result).toBe("<ul><li>0: First</li><li>1: Second</li><li>2: Third</li></ul>");
    });
    ...
    it("ループ内でif条件を使用できる", () => {
      const template: Template = {
        ...
        sectionType: "feature",
        framework: "react",
        content: "<ul>{{#each users}}{{#if active}}<li>{{name}}</li>{{/if}}{{/each}}</ul>",
      };
