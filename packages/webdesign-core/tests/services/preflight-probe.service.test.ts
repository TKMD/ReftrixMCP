...
describe("PreflightProbeService", () => {
  ...
  describe("SSRF Protection", () => {
    ...
    it("should reject IPv6 loopback (::1)", async () => {
      await expect(service.probe("http://[::1]/page")).rejects.toThrow("SSRF protection");
    });

    it("should reject IPv6 link-local (fe80::)", async () => {
      await expect(service.probe("http://[fe80::1]/page")).rejects.toThrow("SSRF protection");
    });

    it("should reject IPv6 unique-local (fc00::, fd00::)", async () => {
      await expect(service.probe("http://[fc00::1]/page")).rejects.toThrow("SSRF protection");
      await expect(service.probe("http://[fd12::1]/page")).rejects.toThrow("SSRF protection");
    });

    it("should reject non-http/https protocols", async () => {
      await expect(service.probe("ftp://example.com")).rejects.toThrow("Invalid protocol");
      await expect(service.probe("file:///etc/passwd")).rejects.toThrow("Invalid protocol");
    });
