...
describe("DB保存連携", () => {
  ...
  it("saveSectionWithEmbedding がセクションIDを返す", async () => {
    ...
    const result = await saveSectionWithEmbedding(sampleSection, mockWebPageId, mockEmbedding);
  ...
  it("正しいEmbeddingフォーマットでDBに保存する", async () => {
    ...
    await saveSectionEmbedding(mockSectionPatternId, mockEmbedding, DEFAULT_MODEL_NAME);
  ...
  it("モデルバージョンがDBに保存される", async () => {
    ...
    await saveSectionEmbedding(mockSectionPatternId, mockEmbedding, "custom-model-v2");
...
describe("統合テスト", () => {
  ...
  it("カスタムオプションでの完全なワークフロー", async () => {
    ...
    // 類似度計算
    const similarity = service.calculateSimilarity(textResult.embedding, sectionResult.embedding);
...
describe("generateBatchWithVision", () => {
  ...
  it("複数のVision付きセクションからEmbeddingを一括生成する", async () => {
    const sections = [
      ...
      sampleSectionWithoutVision,
      sampleSectionWithFailedVision,
    ];
...
const sampleSectionWithPartialVisionFeatures: SectionWithVision = {
  ...
  visionFeatures: {
    ...
    features: [
      ...
      { type: "color_scheme", confidence: 0.8, description: "light theme with blue accents" },
    ],
  },
};
...
describe("convertToVisionFeaturesForEmbedding", () => {
  it("VisionFeatures形式を変換できる", () => {
    const input = {
      ...
      features: [{ type: "layout_structure", confidence: 0.9, description: "test desc" }],
    };
  ...
  it("Vision付きセクションのtextRepresentationが正しく生成・保存される", async () => {
    ...
    const sectionWithVision: SectionWithVision = {
      ...
      visionFeatures: {
        success: true,
        textRepresentation:
          "Dark theme hero section with generous whitespace and clear visual hierarchy",
        features: [
