...
async function createScaledImage(
  ...
  return createImageWithRect(width, height, rectX, rectY, rectSize, rectSize, bgColor, rectColor);
}
...
describe("MotionVectorEstimator", () => {
  ...
  beforeAll(async () => {
    ...
    const { MotionVectorEstimator } =
      await import("../../../../src/services/motion/analyzers/motion-vector.estimator.js");
    estimator = new MotionVectorEstimator();
  });
  ...
  describe("estimateFlow()", () => {
    ...
    it("should calculate avgSpeed correctly for known displacement", async () => {
      ...
      const frame1 = await createImageWithRect(TEST_WIDTH, TEST_HEIGHT, 20, 40, 20, 20);
      const frame2 = await createImageWithRect(
        TEST_WIDTH,
        TEST_HEIGHT,
        20 + displacement,
    ...
    it("should return confidence between 0 and 1", async () => {
      // Arrange
      const frame1 = await createImageWithRect(TEST_WIDTH, TEST_HEIGHT, 20, 40, 20, 20);
      const frame2 = await createImageWithRect(TEST_WIDTH, TEST_HEIGHT, 40, 40, 20, 20);
  ...
  describe("edge cases", () => {
    ...
    it("should handle empty buffer gracefully", async () => {
      ...
      // Act & Assert
      await expect(estimator.estimateFlow(emptyBuffer, validFrame)).rejects.toThrow();
    });
  ...
  describe("performance", () => {
    it("should complete single estimation within 100ms", async () => {
      // Arrange
      const frame1 = await createImageWithRect(TEST_WIDTH, TEST_HEIGHT, 10, 40, 20, 20);
      const frame2 = await createImageWithRect(TEST_WIDTH, TEST_HEIGHT, 30, 40, 20, 20);
      ...
      // Assert: 500ms以内（CI環境の2コアランナーでの変動を考慮）
    ...
    it("should handle large images reasonably", async () => {
      ...
      // Assert: 15000ms以内（大きい画像のため、CI 2コアランナーでの変動を考慮）
      // 通常1-2秒で完了するが、CI環境で8秒超の実績あり
