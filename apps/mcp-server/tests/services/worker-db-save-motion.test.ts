...
describe("saveMotionPatterns", () => {
  ...
  it("should use propertiesDetailed with from/to values when available", async () => {
    const patterns: MotionPatternInput[] = [
      {
        ...
        easing: "ease-out",
        properties: ["opacity"],
        propertiesDetailed: [{ property: "opacity", from: "0", to: "1" }],
      },
    ];
    ...
    const savedProperties = data?.[0]?.properties as Array<{
      property: string;
      from: string;
      to: string;
    }>;
  ...
  it("should return empty result for empty patterns array", async () => {
    const result = await saveMotionPatterns(mockPrisma, "web-page-id", [], "https://example.com");
    ...
    const patterns: MotionPatternInput[] = [
      {
        ...
        propertiesDetailed: [{ property: "background-color", from: "red" }],
      },
    ];
