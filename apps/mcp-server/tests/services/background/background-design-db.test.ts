...
describe("saveBackgroundDesigns", () => {
  ...
  describe("空配列", () => {
    it("空配列の場合は成功を返しDB操作をスキップすること", async () => {
      const result = await saveBackgroundDesigns(mockPrisma, "test-web-page-id", []);
  ...
  describe("エラーハンドリング", () => {
    it("deleteMany 失敗時にエラーを返すこと", async () => {
      mockPrisma.backgroundDesign.deleteMany.mockRejectedValue(new Error("Connection refused"));
  ...
  describe("UUIDv7 生成", () => {
    it("各背景デザインに一意のUUIDv7が割り当てられること", async () => {
      ...
      const result = await saveBackgroundDesigns(mockPrisma, "test-web-page-id", backgrounds);
