...
describe("MotionPatternPersistenceService", () => {
  ...
  describe("savePattern", () => {
    ...
    it("webPageId と sourceUrl が正しく保存される", async () => {
      ...
      const createCall = (mockPrisma.motionPattern.create as ReturnType<typeof vi.fn>).mock
        .calls[0];
  ...
  describe("savePatterns（一括保存）", () => {
    ...
    it("continueOnError=false の場合、エラー発生時に例外をスローする", async () => {
      ...
      await expect(service.savePatterns(patterns, { continueOnError: false })).rejects.toThrow(
        "Mock error on pattern 2"
      );
    });
