...
describe("LayoutSearchService - visionAnalysis 拡張", () => {
  ...
  describe("vectorResultToSearchResult 変換", () => {
    ...
    it("複数の検索結果で visionAnalysis が混在する場合も正しく処理されること", async () => {
      ...
      expect(
        (result?.results[0]?.layoutInfo?.visionAnalysis as ExpectedVisionAnalysis).success
      ).toBe(true);
      ...
      expect(
        (result?.results[2]?.layoutInfo?.visionAnalysis as ExpectedVisionAnalysis).success
      ).toBe(false);
    });
  ...
  describe("visionAnalysis フィールド詳細検証", () => {
    ...
    it("features 配列の各要素が正しく抽出されること", async () => {
      ...
      const visionAnalysis = result?.results[0]?.layoutInfo
        ?.visionAnalysis as ExpectedVisionAnalysis;
      ...
      // その他のフィールド
      expect(visionAnalysis.textRepresentation).toBe(
        "Layout: Two-column grid. Colors: Dark theme."
      );
