...
describe("saveJsAnimationPatterns", () => {
  ...
  describe("CDPAnimationData変換", () => {
    ...
    it("name未設定の場合フォールバック名が生成される", async () => {
      ...
      await saveJsAnimationPatterns(mockPrisma, WEB_PAGE_ID, jsAnimations, SOURCE_URL);
  ...
  describe("WebAnimationData変換", () => {
    ...
    it("キーフレームからプロパティが正しく抽出される", async () => {
      ...
      const call = vi.mocked(mockPrisma.jSAnimationPattern.createMany).mock.calls[0];
      const data = (call?.[0] as { data: Record<string, unknown>[] })?.data;
      const properties = data?.[0]?.properties as Array<{
        property: string;
        from: string;
        to: string;
      }>;
  ...
  describe("クリーンスレート動作", () => {
    it("既存データを削除してから新規作成する", async () => {
      ...
      const deleteManyOrder = vi.mocked(mockPrisma.jSAnimationPattern.deleteMany).mock
        .invocationCallOrder[0];
      const createManyOrder = vi.mocked(mockPrisma.jSAnimationPattern.createMany).mock
        .invocationCallOrder[0];
