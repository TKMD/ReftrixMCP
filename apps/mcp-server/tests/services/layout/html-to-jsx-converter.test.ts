...
describe("convertHtmlToJsx", () => {
  ...
  describe("インラインスタイル変換（useTailwind: true）", () => {
    ...
    it("複合スタイルが正しく変換される", () => {
      const html =
        '<div style="display: flex; justify-content: center; align-items: center; padding: 16px; background-color: black;">Content</div>';
      const result = convertHtmlToJsx(html, { useTailwind: true });
      expect(result).toContain("flex");
      expect(result).toContain("justify-center");
      expect(result).toContain("items-center");
  ...
  describe("TailwindCSS v4.1 対応", () => {
    ...
    it("grid-template-columns が grid-cols-* に変換される", () => {
      const html =
        '<div style="display: grid; grid-template-columns: repeat(3, 1fr);">Content</div>';
      const result = convertHtmlToJsx(html, { useTailwind: true });
