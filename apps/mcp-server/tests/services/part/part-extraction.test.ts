...
describe('isLogoElement', () => {
  it('header内の .logo クラスを持つ SVG をロゴと判定する', () => {
    const dom = new JSDOM('<header><svg class="logo" viewBox="0 0 24 24"><path d="M0 0" /></svg></header>');
  ...
  it('header外の SVG はロゴと判定しない', () => {
    const dom = new JSDOM('<main><svg class="logo" viewBox="0 0 24 24"><path d="M0 0" /></svg></main>');
    const svg = dom.window.document.querySelector('svg') as Element;
    expect(isLogoElement(svg)).toBe(false);
  });

  it('header内でもlogoクラスを持たない要素はロゴと判定しない', () => {
    const dom = new JSDOM('<header><svg class="icon-menu" viewBox="0 0 24 24"><path d="M0 0" /></svg></header>');
    const svg = dom.window.document.querySelector('svg') as Element;
    expect(isLogoElement(svg)).toBe(false);
  });
});
...
describe('DOMPurify sanitization in extractPartsFromSection', () => {
  it('script タグを含むHTMLをサニタイズして除去する', async () => {
    const html = '<div><button onclick="alert(1)">Click</button><script>alert("xss")</script></div>';
