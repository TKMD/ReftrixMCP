...
function createMockDbPart(overrides?: Partial<{
  id: string;
  partType: string;
  cssClasses: string[];
  ...
  describe('resolvePartBoundingBoxes', () => {
    ...
    describe('セクション相対座標変換 / Section-relative coordinate conversion', () => {
      it('sectionStartY を引いた y 座標が page.evaluate に渡されること', async () => {
        ...
        const capturedEvaluateArg = vi.fn().mockResolvedValue([
          { id: MOCK_PART_ID_1, x: 10, y: 50, width: 100, height: 40 },
        ]);
        ...
        const selectorDataArg = capturedEvaluateArg.mock.calls[0][1] as Array<{ sectionStartY: number }>;
      ...
      it('layoutInfo が null の場合 sectionStartY は 0 になること', async () => {
        ...
        const mockPrisma = createMockPrisma({
          componentPartFindMany: vi.fn().mockResolvedValue(mockParts),
          sectionPatternFindMany: vi.fn().mockResolvedValue([
            { id: MOCK_SECTION_ID_1, layoutInfo: null },
          ]),
        });
    ...
    describe('$transaction バッチ更新 / Batch update', () => {
      it('複数パーツが1トランザクションで更新されること', async () => {
        ...
        const mockParts = [
          ...
          createMockDbPart({ id: MOCK_PART_ID_2, partType: 'heading', sectionPatternId: MOCK_SECTION_ID_1 }),
          createMockDbPart({ id: MOCK_PART_ID_3, partType: 'image', sectionPatternId: MOCK_SECTION_ID_2 }),
        ];
  ...
  describe('buildSelectorsForPart', () => {
    describe('タグ + クラス / Tag + class selectors', () => {
      it('button タイプで cssClasses がある場合、tag.class と tag の両方を返す', () => {
        ...
        expect(selectors.indexOf('button.btn.primary')).toBeLessThan(
          selectors.indexOf('button'),
        );
      });
