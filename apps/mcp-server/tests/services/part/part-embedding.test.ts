 ...
 */
function createBasicPart(overrides?: Partial<ComponentPartForEmbedding>): ComponentPartForEmbedding {
  return {
...
describe('generateVisualEmbedding', () => {
  ...
  it('NaN含有ベクトルを拒否する [H-3] / rejects NaN vector [H-3]', async () => {
    ...
    await expect(generateVisualEmbedding(dinov2, cropBuffer))
      .rejects.toThrow('Invalid visual embedding: contains NaN or Infinity');
  });
  ...
  it('ゼロベクトルを拒否する [H-3] / rejects zero vector [H-3]', async () => {
    ...
    await expect(generateVisualEmbedding(dinov2, cropBuffer))
      .rejects.toThrow('Invalid visual embedding: zero vector (L2 norm is 0)');
  });
  ...
  it('次元数が768でない場合にエラーを投げる / throws on dimension mismatch', async () => {
    ...
    await expect(generateVisualEmbedding(dinov2, cropBuffer))
      .rejects.toThrow('Visual embedding dimension mismatch');
  });
});
...
describe('generateTextEmbedding', () => {
  ...
  it('NaN含有ベクトルを拒否する [H-3] / rejects NaN vector [H-3]', async () => {
    ...
    await expect(generateTextEmbedding(embeddingService, 'passage: type:button'))
      .rejects.toThrow('Invalid text embedding: contains NaN or Infinity');
  });
});
