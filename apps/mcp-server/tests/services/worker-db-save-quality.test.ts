...
function createMockPrisma(
  overrides?: Partial<QualityEvaluationPrismaClient["qualityEvaluation"]>
...
function createSampleQualityInput(
...
describe("saveQualityEvaluation", () => {
  ...
  describe("正常保存", () => {
    ...
    it("should create with correct field mapping", async () => {
      ...
      const createCall = (mockPrisma.qualityEvaluation.create as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
    ...
    it("should include antiAiCliche JSON with axisScores and clicheCount", async () => {
      ...
      const input = createSampleQualityInput({
        cliches: [
          {
            type: "gradient_blob",
            description: "Generic gradient blob",
        ...
        recommendations: [
          {
            ...
            description: "Increase text contrast ratio",
          },
          {
            id: "rec-2",
            category: "layout",
