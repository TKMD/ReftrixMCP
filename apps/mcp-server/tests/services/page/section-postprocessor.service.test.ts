...
function makeSection(overrides: Partial<PostProcessableSection> = {}): PostProcessableSection {
  return {
    ...
    heading: undefined,
    htmlSnippet:
      '<div class="section">Some content here for testing purposes that is long enough</div>',
  ...
  describe("Rule 1: consecutive same-type merge", () => {
    ...
    it("should recalculate position from min/max of merged group", () => {
      const sections = [
        makeSection({
          type: "feature",
          position: { startY: 100, endY: 300, height: 200 },
          positionIndex: 0,
        }),
        makeSection({
          type: "feature",
        ...
        makeSection({
          type: "feature",
          position: { startY: 500, endY: 700, height: 200 },
          positionIndex: 2,
        }),
      ];
      const result = postProcessSections(sections);
  ...
  describe("NaN/Infinity defense", () => {
    it("should handle NaN in position gracefully", () => {
      const sections = [
        makeSection({
          type: "feature",
          position: { startY: NaN, endY: 200, height: 200 },
          positionIndex: 0,
        }),
        makeSection({
          type: "feature",
          position: { startY: 200, endY: 400, height: 200 },
          positionIndex: 1,
        }),
        makeSection({
          type: "feature",
          position: { startY: 400, endY: 600, height: 200 },
          positionIndex: 2,
        }),
      ];
      const result = postProcessSections(sections);
    ...
    it("should split oversized section equally when no HTML child elements (Strategy B)", () => {
      ...
      const sections = [
        makeSection({
          ...
          htmlSnippet:
            '<div class="large-content">Lots of plain text content without section or article tags</div>',
        }),
      ];
      const result = postProcessSections(sections);
