...
describe("PlaywrightCrawlerService - Unit Tests", () => {
  ...
  describe("Protocol Validation", () => {
    ...
    it("javascriptプロトコルがブロックされること", async () => {
      await expect(crawlPage("javascript:alert(1)", { timeout: 5000 })).rejects.toThrow();
    });

    it("dataプロトコルがブロックされること", async () => {
      await expect(crawlPage("data:text/html,<h1>test</h1>", { timeout: 5000 })).rejects.toThrow();
    });
  });
...
describe("PlaywrightCrawlerService - withTimeout Protection", () => {
  ...
  it("should wrap page.content() with withTimeout", () => {
    expect(crawlerSource).toContain("withTimeout(page.content(), operationTimeout,");
...
describe("PlaywrightCrawlerService - Integration Tests", () => {
  ...
  describe("HTTP Status Codes", () => {
    it("404レスポンスでエラーをスローすること", async () => {
      await expect(crawlPage("https://httpstat.us/404", { timeout: 30000 })).rejects.toThrow(
        CrawlError
      );
    }, 60000);

    it("500レスポンスでエラーをスローすること", async () => {
      await expect(crawlPage("https://httpstat.us/500", { timeout: 30000 })).rejects.toThrow(
        CrawlError
      );
    }, 60000);
  });
