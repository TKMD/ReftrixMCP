...
describe("OllamaVisionClient", () => {
  ...
  describe("error handling", () => {
    ...
    it("JSONパースエラー時にINVALID_RESPONSEエラーをスローすること", async () => {
      ...
      await expect(client.generateJSON("base64image", "test prompt")).rejects.toMatchObject({
        code: "INVALID_RESPONSE",
        isRetryable: false,
      });
    });
  ...
  describe("generateJSON", () => {
    ...
    it("JSONが埋め込まれたテキストからJSONを抽出すること", async () => {
      ...
      const result = await client.generateJSON<{ result: string }>("base64image", "return json");
  ...
  describe("non-greedy JSON extraction", () => {
    it("複数のJSONオブジェクトがある場合、最初のオブジェクトを抽出すること", async () => {
      ...
      mockFetch.mockResolvedValueOnce({
        ...
        json: async () => ({
          response:
            'Here is the result: {"first": "object"} And here is another example: {"second": "object"}',
        }),
      });

      const result = await client.generateJSON<{ first: string }>("base64image", "test prompt");
...
describe("OllamaVisionClient - Dynamic Timeout (Phase 2)", () => {
  ...
  describe("dynamic timeout calculation", () => {
    it("GPUモードでは60秒タイムアウトを使用すること", async () => {
      ...
      const result = await client.generateWithImageSize("base64image", "test prompt", 600_000);
    ...
    it("CPUモード + 小画像（<100KB）では180秒タイムアウトを使用すること", async () => {
      ...
      // 小さい画像（50KB）
      const result = await client.generateWithImageSize("base64image", "test prompt", 50_000);
    ...
    it("CPUモード + 中画像（100KB-500KB）では600秒タイムアウトを使用すること", async () => {
      ...
      // 中サイズ画像（200KB）
      const result = await client.generateWithImageSize("base64image", "test prompt", 200_000);
    ...
    it("CPUモード + 大画像（>=500KB）では1200秒タイムアウトを使用すること", async () => {
      ...
      // 大きい画像（1MB）
      const result = await client.generateWithImageSize("base64image", "test prompt", 1_000_000);
