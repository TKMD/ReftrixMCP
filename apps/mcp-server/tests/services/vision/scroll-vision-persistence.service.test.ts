...
describe("saveScrollVisionResults", () => {
  ...
  it("should create MotionPattern records from animations", async () => {
    const visionResult = createMockVisionResult(2);
    (mockPrisma.motionPattern.createMany as ReturnType<typeof vi.fn>).mockResolvedValue({
      count: 2,
    });
    ...
    const callArgs = (mockPrisma.motionPattern.createMany as ReturnType<typeof vi.fn>).mock
      .calls[0][0];
