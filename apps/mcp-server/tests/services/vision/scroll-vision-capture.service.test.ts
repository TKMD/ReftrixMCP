...
describe("computeScrollPositions", () => {
  it("ページ先頭(0)を常に含む", () => {
    const boundaries: SectionBoundary[] = [{ sectionIndex: 0, startY: 500, endY: 1000 }];
  ...
  it("50px以内のスクロール位置はマージされる", () => {
    ...
    // 500と530が1つにマージされるはず
    const positionsAt500Area = positions.filter((p) => p.scrollY >= 490 && p.scrollY <= 540);
  ...
  it("50pxを超える距離の位置はマージされない", () => {
    ...
    const positionsInRange = positions.filter((p) => p.scrollY >= 500 && p.scrollY <= 600);
  ...
  it("startYがmaxScrollを超える場合はクランプされる", () => {
    const boundaries: SectionBoundary[] = [{ sectionIndex: 0, startY: 5000, endY: 6000 }];
    const totalScrollHeight = 2000;
    const viewportHeight = 900;
    const maxScroll = totalScrollHeight - viewportHeight; // 1100
  ...
  it("負のstartYは0にクランプされる", () => {
    const boundaries: SectionBoundary[] = [{ sectionIndex: 0, startY: -100, endY: 500 }];
  ...
  it("viewportがtotalScrollHeight以上の場合、maxScrollは0", () => {
    const boundaries: SectionBoundary[] = [{ sectionIndex: 0, startY: 200, endY: 500 }];
...
describe("captureScrollPositions", () => {
  ...
  it("SSRF検証で拒否された場合はエラーをスロー", async () => {
    ...
    const boundaries: SectionBoundary[] = [{ sectionIndex: 0, startY: 0, endY: 500 }];

    await expect(captureScrollPositions("http://192.168.1.1", boundaries)).rejects.toThrow(
      "SSRF blocked"
    );
  });
  ...
  it("エラー時もリソースがクリーンアップされる", async () => {
    ...
    await expect(captureScrollPositions("https://example.com", boundaries)).rejects.toThrow(
      "Navigation failed"
    );
