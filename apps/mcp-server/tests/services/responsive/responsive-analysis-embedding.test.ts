...
function createFullAnalysis(
  overrides?: Partial<ResponsiveAnalysisForText>
): ResponsiveAnalysisForText {
  return {
    id: "ra-001",
...
describe("generateResponsiveAnalysisTextRepresentation", () => {
  ...
  it("should handle differences without selector", () => {
    const analysis = createFullAnalysis({
      differences: [{ category: "visibility", description: "Sidebar hidden on mobile" }],
    });
    const result = generateResponsiveAnalysisTextRepresentation(analysis);
...
describe("generateResponsiveAnalysisEmbeddings", () => {
  ...
  it("should call progress callback", async () => {
    const analyses = [createFullAnalysis({ id: "ra-001" }), createFullAnalysis({ id: "ra-002" })];
  ...
  it("should throw if EmbeddingService factory not set", async () => {
    ...
    await expect(generateResponsiveAnalysisEmbeddings([analysis])).rejects.toThrow(
      "ResponsiveEmbeddingService not initialized"
    );
  });
  ...
  it("should throw if PrismaClient factory not set", async () => {
    ...
    await expect(generateResponsiveAnalysisEmbeddings([analysis])).rejects.toThrow(
      "ResponsivePrismaClient not initialized"
    );
  });
});
