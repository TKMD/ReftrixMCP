  ...
  describe("isAvailable", () => {
    describe("正常系", () => {
      it("Ollamaサーバーとモデルが利用可能な場合trueを返すこと", async () => {
        mockFetch.mockResolvedValueOnce(createTagsResponse(["llama3.2-vision", "llama3.2"]));
      ...
      it("カスタムモデル名でも利用可能性を確認できること", async () => {
        ...
        mockFetch.mockResolvedValueOnce(createTagsResponse(["custom-vision", "other-model"]));
    ...
    describe("異常系", () => {
      ...
      it("モデルが見つからない場合falseを返すこと", async () => {
        mockFetch.mockResolvedValueOnce(createTagsResponse(["other-model", "another-model"]));
  ...
  describe("analyze", () => {
    describe("正常系", () => {
      it("画像解析が成功すること", async () => {
        mockFetch.mockResolvedValueOnce(createGenerateResponse(createValidAnalysisJson()));
...
This is a professional layout.`;
      ...
      mockFetch.mockResolvedValueOnce(createGenerateResponse(responseWithMarkdown));
  ...
  describe("プロンプト生成", () => {
    ...
    it("特徴タイプに基づいてプロンプトが生成されること", async () => {
      ...
      const features: VisionFeatureType[] = ["layout_structure", "whitespace"];
  ...
  describe("エッジケース", () => {
    ...
    it("複数の同時リクエストが処理できること", async () => {
      mockFetch.mockResolvedValue(createGenerateResponse(createValidAnalysisJson()));
