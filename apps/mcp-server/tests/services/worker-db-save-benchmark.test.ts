...
describe("saveQualityBenchmarks", () => {
  ...
  describe("ページ全体の品質ベンチマーク保存", () => {
    it("should save page-level benchmark with web_page_id and return SaveResult", async () => {
      ...
      const result = await saveQualityBenchmarks(mockPrisma, webPageId, [input]);
    ...
    it("should set web_page_id on page-level benchmark data", async () => {
      ...
      const createCall = (mockPrisma.qualityBenchmark.createMany as ReturnType<typeof vi.fn>).mock
        .calls[0][0];
  ...
  describe("複数ベンチマーク一括保存", () => {
    it("should save multiple benchmarks in a single createMany call", async () => {
      ...
      const inputs = [
        ...
        createSectionBenchmarkInput({
          sectionType: "cta",
          overallScore: 86,
          sectionPatternId: "section-pattern-uuid-002",
        }),
      ];

      const result = await saveQualityBenchmarks(mockPrisma, webPageId, inputs);
    ...
    it("should generate unique UUIDv7 for each benchmark", async () => {
      ...
      const inputs = [createPageBenchmarkInput(), createSectionBenchmarkInput()];
  ...
  describe("クリーンスレート（deleteMany → createMany）", () => {
    it("should delete existing benchmarks for the web_page_id before creating", async () => {
      ...
      const deleteManyOrder = (mockPrisma.qualityBenchmark.deleteMany as ReturnType<typeof vi.fn>)
        .mock.invocationCallOrder[0];
      const createManyOrder = (mockPrisma.qualityBenchmark.createMany as ReturnType<typeof vi.fn>)
        .mock.invocationCallOrder[0];
  ...
  describe("空配列/データなしのハンドリング", () => {
    it("should return success with count 0 when benchmarks array is empty", async () => {
      ...
      const result = await saveQualityBenchmarks(mockPrisma, webPageId, []);
