...
describe("page.analyze Smoke Test", () => {
  ...
  describe("入力スキーマ構造", () => {
    const properties = pageAnalyzeToolDefinition.inputSchema.properties as Record<string, unknown>;
  ...
  describe("page.getJobStatus ツール定義", () => {
    ...
    it("page.getJobStatus がエクスポートされている", async () => {
      const { pageGetJobStatusToolDefinition } = await import("../../src/tools/page");
