...
vi.mock("../../../src/services/motion/motion-db.service", () => {
  const mockSaveFrameAnalysis = vi
    .fn()
    .mockImplementation((input: FrameImageAnalysisOutput): Promise<BatchSaveResult> => {
      const totalItems =
      ...
      return Promise.resolve({
        ...
        embeddingIds:
          totalItems > 0
            ? Array.from(
                { length: totalItems },
                () => `mock-embedding-${Math.random().toString(36).substring(7)}`
              )
            : [],
      });
    });
  ...
  return {
    getMotionDbService: vi.fn().mockReturnValue({
      ...
      saveAnimationZone: vi
        .fn()
        .mockResolvedValue({ resultId: "mock-result-id", embeddingId: "mock-embedding-id" }),
      saveLayoutShift: vi
        .fn()
        .mockResolvedValue({ resultId: "mock-result-id", embeddingId: "mock-embedding-id" }),
      saveMotionVector: vi
...
describe("VideoMode DB保存統合テスト", () => {
  ...
  describe("saveFrameAnalysis", () => {
    it("frameAnalysisを正常に保存できる", async () => {
      ...
      // Act
      const result: BatchSaveResult = await motionDbService.saveFrameAnalysis(frameAnalysisOutput, {
    ...
    it("空のframeAnalysisでも正常に処理される", async () => {
      ...
      // Act
      const result: BatchSaveResult = await motionDbService.saveFrameAnalysis(emptyOutput, {
  ...
  describe("エラーハンドリング（Graceful Degradation）", () => {
    ...
    it("無効な入力でもエラーを投げない", async () => {
      ...
      const result = await motionDbService.saveFrameAnalysis(invalidOutput, {
  ...
  describe("パフォーマンス", () => {
    it("大量のデータでも妥当な時間内に処理される", async () => {
      ...
      // Act
      const result = await motionDbService.saveFrameAnalysis(largeOutput, {
    ...
    it("MotionServiceResult形式からFrameImageAnalysisOutput形式に正しく変換される", () => {
      ...
      if (significantFrames.length > 0) {
        ...
        for (let i = 0; i < significantFrames.length; i++) {
          ...
          const timelineEntry = motionServiceFrameAnalysis.timeline.find(
            (t) => t.frame_index === currentFrame
          );
          if (timelineEntry) {
    ...
    it("例外発生時にFRAME_ANALYSIS_DB_SAVE_ERRORがwarningsに追加される", () => {
      ...
      try {
        ...
        warnings.push({
          ...
          message:
            frameDbError instanceof Error ? frameDbError.message : "Frame analysis DB save failed",
        });
      }
