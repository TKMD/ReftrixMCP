...
const VALID_DATABASE_URL_WITH_SSL =
...
class MockDatabaseDumper implements DatabaseDumper {
  ...
  setFailure(
    shouldFail: boolean,
    reason: "connection" | "dump" | "restore" | "timeout" = "dump"
  ): void {
...
describe("LocalStorageProvider + BackupService 統合テスト", () => {
  ...
  describe("バックアップリストアフロー", () => {
    ...
    it("チェックサムが一致しない場合エラー", async () => {
      ...
      // Act & Assert
      await expect(backupService.restoreBackup(backup.id, VALID_DATABASE_URL)).rejects.toThrow(
        "checksum mismatch"
      );
    });
  });
...
describe("エラーハンドリング統合テスト", () => {
  ...
  describe("DatabaseDumperエラー", () => {
    it("ダンプ失敗時にバックアップステータスがfailedになる", async () => {
      ...
      // Act & Assert
      await expect(backupService.createBackup(VALID_DATABASE_URL)).rejects.toThrow();
    ...
    it("タイムアウトエラー時に適切なエラーが伝播する", async () => {
      ...
      // Act & Assert
      await expect(backupService.createBackup(VALID_DATABASE_URL)).rejects.toThrow("timed out");
    });
...
describe("SecureLocalStorageProvider 統合テスト", () => {
  ...
  describe("セキュリティ", () => {
    it("パストラバーサル攻撃を防ぐ", async () => {
      // Act & Assert
      await expect(provider.upload("../escape.txt", Buffer.from("attack"))).rejects.toThrow(
        StorageError
      );

      await expect(provider.download("../../etc/passwd")).rejects.toThrow(StorageError);
    });

    it("絶対パスを拒否する", async () => {
      // Act & Assert
      await expect(provider.upload("/etc/passwd", Buffer.from("attack"))).rejects.toThrow(
        StorageError
      );
    });
    ...
    it("空のキーを拒否する", async () => {
      // Act & Assert
      await expect(provider.upload("", Buffer.from("data"))).rejects.toThrow(StorageError);
    });

    it("URLエンコードされたパストラバーサルを検出する", async () => {
      // Act & Assert
      await expect(provider.upload("%2e%2e/escape.txt", Buffer.from("attack"))).rejects.toThrow(
        StorageError
      );
    });
  });
...
describe("DatabaseDumperService URL解析 統合テスト", () => {
  describe("parseDatabaseUrl", () => {
    ...
    it("無効なURLでエラーを投げる", () => {
      ...
      expect(() => parseDatabaseUrl("mysql://user:pass@localhost/db")).toThrow(
        DatabaseUrlParseError
      );
