...
function createTestSections() {
  return [
    ...
    },
    {
      ...
      positionIndex: 3,
      confidence: 0.9,
      position: { startY: 1300, endY: 1800, height: 500 },
    },
    {
      id: "section-cta",
      type: "cta",
      positionIndex: 4,
      confidence: 0.92,
      position: { startY: 1800, endY: 2000, height: 200 },
    },
    {
      id: "section-footer",
      type: "footer",
      positionIndex: 5,
      confidence: 0.85,
      position: { startY: 2000, endY: 2150, height: 150 },
    },
  ];
}
...
function createMockVisionResult(sectionType: string, sectionId: string) {
  const typeSpecificFeatures: Record<
    string,
    Array<{ type: string; confidence: number; data: { description: string } }>
  > = {
    hero: [
      {
        type: "layout_structure",
        confidence: 0.92,
        data: { description: "Centered hero layout with gradient background" },
      },
      {
        type: "color_palette",
        confidence: 0.88,
        data: { description: "Purple-blue gradient, white text" },
      },
    ...
    feature: [
      {
        type: "layout_structure",
        confidence: 0.89,
        data: { description: "Grid layout with 2 feature cards" },
      },
      {
        type: "color_palette",
        confidence: 0.84,
        data: { description: "Light gray background, dark text" },
      },
      {
        type: "whitespace",
        confidence: 0.81,
        data: { description: "Balanced spacing between cards" },
      },
    ],
    testimonial: [
      {
        type: "layout_structure",
        confidence: 0.87,
        data: { description: "Single column with blockquote" },
      },
      {
        type: "color_palette",
        confidence: 0.83,
        data: { description: "White background, emphasis on quote" },
      },
    ...
    pricing: [
      {
        type: "layout_structure",
        confidence: 0.91,
        data: { description: "Card-based pricing table" },
      },
      {
        type: "color_palette",
        confidence: 0.86,
        data: { description: "Subtle gray background, highlighted price" },
      },
      {
        type: "whitespace",
        confidence: 0.82,
        data: { description: "Clear separation between elements" },
      },
    ],
    cta: [
      {
        type: "layout_structure",
        confidence: 0.93,
        data: { description: "Centered CTA with prominent button" },
      },
      {
        type: "color_palette",
        confidence: 0.9,
        data: { description: "Dark blue background, white button" },
      },
      {
        type: "whitespace",
        confidence: 0.88,
        data: { description: "Focused, attention-grabbing" },
      },
    ],
    footer: [
      {
        type: "layout_structure",
        confidence: 0.84,
        data: { description: "Multi-column footer navigation" },
      },
      {
        type: "color_palette",
        confidence: 0.8,
        data: { description: "Dark background, light text links" },
      },
...
describe("page.analyze perSectionVision 統合テスト", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(SectionScreenshotService).mockImplementation(
      () => mockScreenshotService as unknown as SectionScreenshotService
    );
    ...
    vi.mocked(LlamaVisionAdapter).mockImplementation(
      () => mockVisionAdapter as unknown as LlamaVisionAdapter
    );
  });
  ...
  describe("各セクションに異なるvisionFeaturesが付与される", () => {
    it("6セクションのページで、各セクションに固有のvisionFeaturesが設定される", async () => {
      ...
      mockVisionAdapter.analyzeSection.mockImplementation(
        async (options: { sectionId: string; sectionTypeHint?: string }) => {
      ...
      mockVisionAdapter.generateSectionTextRepresentation.mockImplementation(
        (
          result: {
            success: boolean;
            features: Array<{ type: string; confidence: number; data: { description: string } }>;
          },
          sectionType: string
        ) => {
      ...
      for (const section of sections) {
        ...
        const textRepresentation = mockVisionAdapter.generateSectionTextRepresentation(
          visionResult,
          section.type
        );
      ...
      expect(
        heroSection?.visionFeatures.features.some((f) => f.data.description.includes("gradient"))
      ).toBe(true);

      const featureSection = sectionResults.find((s) => s.type === "feature");
      expect(featureSection?.visionFeatures.success).toBe(true);
      expect(
        featureSection?.visionFeatures.features.some((f) =>
          f.data.description.includes("Grid layout")
        )
      ).toBe(true);
      ...
      expect(
        ctaSection?.visionFeatures.features.some((f) => f.data.description.includes("CTA"))
      ).toBe(true);
    ...
    it("各セクションのtextRepresentationがセクションタイプを含む", async () => {
      ...
      for (const section of sections) {
        ...
        const textRep = mockVisionAdapter.generateSectionTextRepresentation(
          visionResult,
          section.type
        );
    ...
    it("visionBatchSizeに従ってバッチ処理される", async () => {
      ...
      for (let i = 0; i < sections.length; i += batchSize) {
        batches.push(
          sections.slice(i, i + batchSize).map((s) => ({ sectionId: s.id, type: s.type }))
        );
      }
  ...
  describe("スクリーンショート切り出しエラーハンドリング", () => {
    it("一部のセクション切り出しが失敗してもエラー情報が設定される", async () => {
      ...
      // エラー情報が適切に含まれている
      const ctaError = extractResult.errors.find(
        (e: { sectionId: string }) => e.sectionId === "section-cta"
      );
      expect(ctaError?.errorMessage).toContain("bounds exceed");

      const footerError = extractResult.errors.find(
        (e: { sectionId: string }) => e.sectionId === "section-footer"
      );
    ...
    it("空のBase64画像でも処理が継続する", async () => {
      ...
      mockScreenshotService.extractMultipleSections.mockResolvedValue({
        successes: [],
        errors: [{ sectionId: "section-hero", errorMessage: "Empty image data" }],
      });
    ...
    it("position情報がないセクションはperSectionVision対象外", async () => {
      ...
      const sectionsWithPartialPosition = [
        ...
        { id: "section-feature", type: "feature", positionIndex: 1, confidence: 0.88 }, // position なし
        {
          ...
          positionIndex: 2,
          confidence: 0.92,
          position: { startY: 600, endY: 800, height: 200 },
        },
      ];
  ...
  describe("perSectionVision無効時の動作", () => {
    it("perSectionVision=false の場合、セクション単位Vision分析はスキップされる", async () => {
      ...
      mockVisionAdapter.analyzeSection.mockResolvedValue(
        createMockVisionResult("hero", "section-hero")
      );
...
describe("visionFeatures構造検証", () => {
  ...
  it("成功時のvisionFeaturesが正しい構造を持つ", async () => {
    ...
    const expectedStructure = {
      success: true,
      features: [{ type: "layout_structure", confidence: 0.9, data: { description: "test" } }],
