...
describe('motion.detect WebGL Animation Detection 統合テスト', () => {
  ...
  describe('Animation Category Classification', () => {
    ...
    it('should classify particle animation', async () => {
      ...
      // Assert
      if (result.patterns.length > 0) {
        // particle systemはparticle, noise, complex, parallax, wave, fade, pulse, unknownのいずれかに分類される
        // 視覚的特徴によっては異なるカテゴリに分類される場合がある
        // (fadeは透明度変化がパーティクルのようにみえる場合、pulseは周期的な高低差パターン)
        expect(['particle', 'noise', 'complex', 'parallax', 'wave', 'fade', 'pulse', 'unknown']).toContain(
