...
const partialBrief: Brief = {
  ...
  description:
    "A brief description for testing purposes with enough characters to pass minimum length validation check.",
...
const strictModeCompliantBrief: Brief = {
  ...
  references: [{ url: "https://example.com/ref1" }, { url: "https://example.com/ref2" }],
};
...
describe("BriefValidateService", () => {
  ...
  describe("validate() - strictMode validation", () => {
    it("should generate error for description < 100 chars in strict mode", async () => {
      ...
      expect(
        result.issues.some(
          (i) => i.field === "description" && i.severity === "error" && i.message.includes("100")
        )
      ).toBe(true);
    });
    ...
    it("should generate error for < 2 references in strict mode", async () => {
      ...
      expect(
        result.issues.some(
          (i) => i.field === "references" && i.severity === "error" && i.message.includes("2")
        )
      ).toBe(true);
    });
  ...
  describe("validate() - suggestions generation", () => {
    ...
    it("should generate suggestions in Japanese", async () => {
      ...
      const hasJapanese = result.suggestions.some((s) =>
        /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(s)
      );
  ...
  describe("validate() - issue message content", () => {
    ...
    it("should generate Japanese messages for issues", async () => {
      ...
      const hasJapanese = result.issues.some((i) =>
        /[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF]/.test(i.message)
      );
      expect(hasJapanese).toBe(true);
    });
  });
