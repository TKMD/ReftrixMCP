...
describe("motion.detect -> motion.search 統合テスト (TDD Red Phase)", () => {
  ...
  describe("motion.detect -> motion.search統合: save_to_dbオプション有効時", () => {
    ...
    it.each([
      {
        html: "fadeIn",
        query: "fadeIn opacity transform",
        patternFinder: (p: MotionPattern) => p.name === "fadeIn" || p.category === "scroll_trigger",
        resultFinder: (r: { pattern: MotionPattern }) => r.pattern.name === "fadeIn",
      },
      {
        html: "spinner",
        query: "loading spinner infinite",
        patternFinder: (p: MotionPattern) =>
          p.name === "spin" || p.animation.iterations === "infinite",
        resultFinder: (r: { pattern: MotionPattern }) =>
          r.pattern.name === "spin" || r.pattern.animation.iterations === "infinite",
      },
    ])(
      "検出した$htmlパターンがsearch結果に含まれるべき",
      async ({ html, query, patternFinder, resultFinder }) => {
        ...
        setupSearchServices(
          ...
          createMockPrismaWithPatterns(detectData.patterns)
        );
