...
describe("motion.detect URL mode WebPage自動作成", () => {
  ...
  describe("WebPageService.findOrCreateByUrl", () => {
    it("既存のWebPageがある場合は再利用する (created: false)", async () => {
      ...
      expect(mockWebPageService.findOrCreateByUrl).toHaveBeenCalledWith("https://example.com/test");
    });
    ...
    it("WebPageが存在しない場合は新規作成する (created: true)", async () => {
      ...
      expect(mockWebPageService.findOrCreateByUrl).toHaveBeenCalledWith("https://example.com/new");
    });
