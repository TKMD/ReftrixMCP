...
describe("Motion検出タイムアウトハンドリング", () => {
  describe("タイムアウトパラメータ", () => {
    it("motion.detectツールがtimeoutパラメータを受け付ける", async () => {
      // 動的インポートでスキーマを確認
      const { motionDetectInputSchema } = await import("../../../src/tools/motion/detect.tool");
    ...
    it("タイムアウトのデフォルト値は180秒（3分）", async () => {
      const { DEFAULT_MOTION_TIMEOUT } = await import("../../../src/tools/motion/detect.tool");

      expect(DEFAULT_MOTION_TIMEOUT).toBe(180000); // 3 minutes
    });

    it("タイムアウトの最小値は30秒", async () => {
      const { MIN_MOTION_TIMEOUT } = await import("../../../src/tools/motion/detect.tool");
  ...
  describe("createTimeoutResponse関数", () => {
    it("タイムアウト時にエラーではなく成功レスポンスを返す", async () => {
      const { createTimeoutResponse, TimeoutWarningCode } =
        await import("../../../src/tools/motion/detect.tool");
    ...
    it("タイムアウト時でも部分的なパターンが返される", async () => {
      ...
      // PARTIAL_RESULT警告がある
      const warnings = result.data?.warnings ?? [];
      const hasPartialWarning = warnings.some((w) => w.code === TimeoutWarningCode.PARTIAL_RESULT);
      expect(hasPartialWarning).toBe(true);
    });

    it("タイムアウト警告にはphase情報が含まれる", async () => {
      const { createTimeoutResponse } = await import("../../../src/tools/motion/detect.tool");

      const result = createTimeoutResponse("video", 5000, []);

      const warnings = result.data?.warnings ?? [];
      const timeoutWarning = warnings.find((w) => w.code?.includes("TIMEOUT"));
  ...
  describe("MotionTimeoutErrorクラス", () => {
    it("フェーズと経過時間を保持する", async () => {
      const { MotionTimeoutError } = await import("../../../src/tools/motion/detect.tool");
  ...
  describe("withTimeout関数", () => {
    it("正常完了時は結果を返す", async () => {
      const { withTimeout } = await import("../../../src/tools/motion/detect.tool");
    ...
    it("タイムアウト時はMotionTimeoutErrorをスローする", async () => {
      const { withTimeout, MotionTimeoutError } =
        await import("../../../src/tools/motion/detect.tool");
      ...
      await expect(withTimeout(neverResolve, 50, "test", startTime)).rejects.toThrow(
        MotionTimeoutError
      );
    });
  });
