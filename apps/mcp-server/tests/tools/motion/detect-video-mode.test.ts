...
const mockAnalyzeResultWithMotion: AnalyzeResult = {
  ...
  motionCoverage: 0.4, // 2 segments * 1000ms / 5000ms = 0.40
  durationMs: 5000,
...
describe("detection_mode: video 機能テスト", () => {
  ...
  describe("正常系", () => {
    ...
    it("サマリー情報にvideo固有のメタデータが含まれる", async () => {
      ...
      if (result.success) {
        ...
        expect(result.data.video_info?.motion_coverage).toBe(0.4);
      }
    });
  });
  ...
  describe("エラーハンドリング", () => {
    ...
    it("フレーム解析エラー時に適切なエラーを返す", async () => {
      ...
      mockFrameAnalyzerService.analyzeMotion.mockRejectedValue(new Error("FFmpeg not found"));
    ...
    it("エラー時もクリーンアップが実行される", async () => {
      mockVideoRecorderService.record.mockRejectedValue(new Error("Recording failed"));
