  ...
  describe('正常系 / Success cases', () => {
    ...
    it('include_html=falseの場合はhtmlSnippetがnull', async () => {
      ...
      mockPrismaClient.$queryRawUnsafe = vi.fn().mockResolvedValue([
        { ...createMockPartRow(), html_snippet: null },
      ]);
  ...
  describe('エラーハンドリング / Error handling', () => {
    it('DB障害時にサニタイズされたエラーを返すこと', async () => {
      mockPrismaClient.$queryRawUnsafe = vi.fn().mockRejectedValue(
        new Error('database connection refused on port 26432')
      );
    ...
    it('Prisma P2002エラーがサニタイズされること', async () => {
      const prismaError = new Error('Unique constraint violation on field: id') as Error & { code: string };
