...
// コンテンツ検証ヘルパー（重複削減）
const getHeadingTexts = (section: ReturnType<typeof findSection>) =>
  section?.content.headings.map((h) => h.text) ?? [];
const getParagraphs = (section: ReturnType<typeof findSection>) =>
  section?.content.paragraphs ?? [];
const getButtonTexts = (section: ReturnType<typeof findSection>) =>
  section?.content.buttons.map((b) => b.text) ?? [];
const getLinkHrefs = (section: ReturnType<typeof findSection>) =>
  section?.content.links.map((l) => l.href) ?? [];
...
describe("TDD Red Phase: セクション別コンテンツ分離", () => {
  ...
  describe("extractSectionContent関数の直接テスト", () => {
    it.each([
      ...
      "extractSectionContentは$sectionTypeで$expectedのみ返すべき",
      ({ sectionType, expected, notExpected }) => {
    ...
    it("同じHTMLに対して異なるセクションタイプで呼び出すと異なる結果を返すべき", () => {
      ...
      const comparisons = [
        {
          a: heroContent.headings.map((h) => h.text),
          b: featuresContent.headings.map((h) => h.text),
        },
        { a: heroContent.paragraphs, b: featuresContent.paragraphs },
        {
          a: heroContent.buttons.map((b) => b.text),
          b: featuresContent.buttons.map((b) => b.text),
        },
      ];
