...
describe("エッジケース", () => {
  ...
  describe("大文字小文字", () => {
    it("大文字のスクリプト名も検出する", () => {
      ...
      // 大文字でも検出可能であること
      expect(
        result.indicators.htmlIndicators.length + result.indicators.scriptIndicators.length
...
describe("パフォーマンス", () => {
  ...
  it("1000回のURL分析が100ms以内で完了する", () => {
    const urls = ["https://resn.co.nz", "https://google.com", "https://example.com/webgl/demo"];
