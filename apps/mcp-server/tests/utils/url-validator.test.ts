  ...
  describe("validateExternalUrl", () => {
    ...
    describe("SSRF対策: ブロックされるIPレンジ", () => {
      it("10.0.0.0/8 (クラスAプライベート) をブロックする", () => {
        const testCases = ["http://10.0.0.1", "http://10.255.255.255", "http://10.1.2.3:8080/api"];
      ...
      it("172.16.0.0/12 (クラスBプライベート) をブロックする", () => {
        const testCases = ["http://172.16.0.1", "http://172.31.255.255", "http://172.20.10.5:3000"];
      ...
      it("127.0.0.0/8 (ループバック) をブロックする", () => {
        const testCases = ["http://127.0.0.1", "http://127.255.255.255", "http://127.0.0.2:8080"];
    ...
    describe("無効なURL形式", () => {
      ...
      it("不正な形式のURLを拒否する", () => {
        const testCases = ["https://", "https://.", "https://..", "https:// "];
      ...
      it("URLエンコードされたlocalhostを検出してブロックする", () => {
        const testCases = ["https://localhost%00.example.com", "https://127.0.0.1%00@example.com"];
  ...
  describe("normalizeUrlForValidation", () => {
    ...
    describe("デフォルトポート除去", () => {
      it("https:// のデフォルトポート 443 を除去する", () => {
        ...
        expect(normalizeUrlForValidation("https://example.com:443/path")).toBe(
      ...
      it("http:// のデフォルトポート 80 を除去する", () => {
        ...
        expect(normalizeUrlForValidation("http://example.com:80/path")).toBe(
          "http://example.com/path"
        );
      });

      it("非デフォルトポートは保持する", () => {
        expect(normalizeUrlForValidation("https://example.com:8443")).toBe(
          "https://example.com:8443"
        );
        expect(normalizeUrlForValidation("http://example.com:8080")).toBe(
    ...
    describe("クエリパラメータのソート", () => {
      it("クエリパラメータをアルファベット順にソートする", () => {
        expect(normalizeUrlForValidation("https://example.com?b=2&a=1")).toBe(
          "https://example.com?a=1&b=2"
        );
        expect(normalizeUrlForValidation("https://example.com?z=3&m=2&a=1")).toBe(
      ...
      it("クエリパラメータの値をデコードしない（元の形式を維持）", () => {
        expect(normalizeUrlForValidation("https://example.com?q=%E3%83%86%E3%82%B9%E3%83%88")).toBe(
          "https://example.com?q=%E3%83%86%E3%82%B9%E3%83%88"
        );
      });
    });
    ...
    describe("フラグメント除去", () => {
      it("URLフラグメント（#hash）を除去する", () => {
        expect(normalizeUrlForValidation("https://example.com#section")).toBe(
          "https://example.com"
        );
        expect(normalizeUrlForValidation("https://example.com/path#section")).toBe(
          "https://example.com/path"
        );
        expect(normalizeUrlForValidation("https://example.com/path?q=1#section")).toBe(
    ...
    describe("パス正規化", () => {
      ...
      it("パス内の連続スラッシュを単一スラッシュに正規化する", () => {
        expect(normalizeUrlForValidation("https://example.com//path//to//page")).toBe(
          "https://example.com/path/to/page"
        );
      });

      it("パス内の ./ を除去する", () => {
        expect(normalizeUrlForValidation("https://example.com/./path/./to")).toBe(
          "https://example.com/path/to"
        );
      });

      it("パス内の ../ を解決する", () => {
        expect(normalizeUrlForValidation("https://example.com/a/b/../c")).toBe(
  ...
  describe("SSRF対策: IPv6ブロック", () => {
    ...
    describe("IPv6ユニークローカルアドレス (fc00::/7 - ULA)", () => {
      ...
      it("fdff:ffff:ffff:ffff:ffff:ffff:ffff:ffff をブロックする", () => {
        const result = validateExternalUrl("http://[fdff:ffff:ffff:ffff:ffff:ffff:ffff:ffff]/");
        expect(result.valid).toBe(false);
        expect(result.error).toContain("blocked");
      });
    ...
    describe("Teredo Tunneling (2001:0::/32)", () => {
      ...
      it("2001:0000:4136:e378:8000:63bf:3fff:fdd2 をブロックする", () => {
        // 典型的なTeredo アドレス
        const result = validateExternalUrl("http://[2001:0000:4136:e378:8000:63bf:3fff:fdd2]/");
        expect(result.valid).toBe(false);
        expect(result.error).toContain("blocked");
      });
