...
import type { McpToolError } from "../../src/utils/error-messages";
import {
  formatZodError,
...
const SCHEMAS = {
  ...
  nested: z.object({
    filters: z
      .object({ category: z.string().optional(), style: z.enum(["flat", "line"]).optional() })
      .optional(),
    options: z
      .object({
        viewport: z
          .object({ width: z.number().min(320).max(4096), height: z.number().min(240).max(16384) })
          .optional(),
      })
      .optional(),
  }),
  complex: z.object({
    ...
    purpose: z
      .enum(["icon", "illustration", "mascot", "diagram", "decoration", "other"])
      .optional(),
...
describe("Error Messages Utility", () => {
  ...
  describe.each([
    {
      locale: "en" as const,
      input: "invalid-uuid",
      expectedContains: ["Invalid UUID format", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"],
    },
    ...
    it(`無効なUUID形式に対して読みやすいエラーメッセージを返すこと`, () => {
      ...
      expectedContains.forEach((expected) =>
        expect(formattedErrors[0].message).toContain(expected)
      );
    });
  });
  ...
  describe.each([
    { locale: "en" as const, field: "name", expectedMessage: "Required field 'name' is missing" },
    {
      locale: "ja" as const,
      field: "query",
      expectedMessage: "必須フィールド 'query' が指定されていません",
    },
  ])(
    "formatZodError - 必須フィールド欠落 ($locale, $field)",
    ({ locale, field, expectedMessage }) => {
  ...
  describe.each([
    ...
    },
    {
      ...
      field: "limit",
      expectedContains: "最大: 50",
      detailKey: "max",
      detailValue: 50,
    },
  ])(
    "formatZodError - 範囲外の値 ($locale)",
    ({ locale, input, field, expectedContains, detailKey, detailValue }) => {
  ...
  describe.each([
    {
      locale: "en" as const,
      input: { style: "invalid", purpose: "icon" },
      field: "style",
      expected: "Invalid value 'invalid'. Expected one of: flat, line, filled, gradient",
    },
    {
      locale: "ja" as const,
      input: { style: "flat", purpose: "wrong" },
      field: "purpose",
      expected: "値 'wrong' は無効です。次のいずれかを指定してください: icon, illustration, mascot",
    },
  ...
  describe.each([
    {
      ...
      expectedContains: "at least 1 character",
    },
    {
      locale: "ja" as const,
      input: { name: "a".repeat(250) },
      code: "STRING_TOO_LONG",
      expectedContains: "200文字以下",
    },
  ])(
    "formatZodError - 文字列長エラー ($locale, $code)",
    ({ locale, input, code, expectedContains }) => {
  ...
  describe.each([
    {
      locale: "en" as const,
      input: { count: "not a number", enabled: true, tags: [] },
      field: "count",
      expectedContains: ["Expected number", "received string"],
    },
    {
      locale: "ja" as const,
      input: { count: 5, enabled: true, tags: "not an array" },
      field: "tags",
      expectedContains: ["array"],
    },
  ])(
    "formatZodError - 型エラー ($locale, $field)",
    ({ locale, input, field, expectedContains }) => {
  ...
  describe("formatZodError - ネストしたパス", () => {
    it("ネストしたフィールドのパスが正しく表示されること", () => {
      const error = parseAndExpectError(SCHEMAS.nested, {
        filters: { style: "invalid" },
        options: { viewport: { width: 100, height: 500 } },
      });
      const formattedErrors = formatZodError(error);
  ...
  describe("createValidationError", () => {
    it("ZodErrorからMcpToolError配列を生成できること", () => {
      const error = parseAndExpectError(
        z.object({ query: z.string(), limit: z.number().min(1).max(50) }),
        { limit: 0 }
      );
...
describe("Enhanced Error Messages - formatDetailedValidationError", () => {
  ...
  describe.each([
    {
      ...
      input: { name: "test", license_spdx: "MIT" },
      field: "svg_content",
    ...
    {
      name: "範囲エラー",
      schema: z.object({ quality_threshold: z.number().min(0).max(100) }),
      input: { quality_threshold: 150 },
  ...
  describe("配列バリデーションエラー", () => {
    it("フィールドパスを正しく表示する", () => {
      ...
      const error = parseAndExpectError(schema, {
        items: [
          { svg_content: "", name: "test", license_spdx: "MIT" },
          { svg_content: "<svg></svg>", name: "", license_spdx: "MIT" },
        ],
      });
      const detailed = formatDetailedValidationError(error);
  ...
  describe.each([
    {
      name: "URL",
      schema: z.object({ source_url: z.string().url() }),
      input: { source_url: "not-a-valid-url" },
      field: "source_url",
      hintContains: "https://",
    },
    {
      ...
      input: { id: "not-a-uuid" },
      field: "id",
      hintContains: "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
    },
    {
      name: "enum",
      schema: z.object({ style: z.enum(["flat", "line", "filled", "gradient"]) }),
      input: { style: "invalid-style" },
      field: "style",
      expectedContains: ["flat", "gradient"],
    },
    ...
    it("正しい形式例/選択肢を含む", () => {
      ...
      if (expectedContains)
        expectedContains.forEach((exp) => expect(detailed.errors[0].expected).toContain(exp));
    });
  });
  ...
  describe("深くネストされたオブジェクト", () => {
    it("エラーパスを正しく処理する", () => {
      const schema = z.object({
        options: z.object({
          viewport: z.object({ width: z.number().min(320), height: z.number().min(240) }),
        }),
      });
      const error = parseAndExpectError(schema, {
        options: { viewport: { width: 100, height: 100 } },
      });
      const detailed = formatDetailedValidationError(error);
  ...
  describe.each([
    {
      ...
      schema: SCHEMAS.ingest,
      input: {},
      hintCheck: (hints: string[]) => hints.every((h) => h),
    },
    {
      toolName: "layout.ingest",
      schema: z.object({ url: z.string().url() }),
      input: { url: "invalid" },
      hintCheck: (hints: string[]) => hints.some((h) => h.includes("URL")),
    },
    {
      toolName: "motion.detect",
      schema: z
        .object({ pageId: z.string().uuid().optional(), html: z.string().optional() })
        .refine((d) => d.pageId || d.html, {
          message: "pageIdまたはhtmlのいずれかを指定してください",
        }),
      input: {},
      hintCheck: (hints: string[]) =>
        hints.some((h) => h?.includes("pageId") || h?.includes("html")),
    },
...
describe("Enhanced Error Messages - formatSingleError/formatMultipleErrors", () => {
  it("ヒント付きのエラーを整形する", () => {
    const error: McpToolError & { hint?: string } = {
      code: ERROR_CODES.REQUIRED_FIELD,
      message: "Required field 'svg_content' is missing",
      field: "svg_content",
    };
...
describe("refine によるカスタムバリデーションエラー", () => {
  it("refine エラーを処理する", () => {
    const schema = z
      .object({ colors: z.record(z.string()).optional(), apply_palette: z.boolean().optional() })
      .refine((d) => (d.colors && Object.keys(d.colors).length > 0) || d.apply_palette === true, {
