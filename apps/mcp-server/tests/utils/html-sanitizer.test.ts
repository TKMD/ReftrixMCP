...
describe("HTMLサニタイザー", () => {
  ...
  describe("基本機能", () => {
    ...
    it("安全なHTMLを保持する", () => {
      // Arrange: 安全なHTML
      const safeHtml = '<div class="container"><p>Hello <strong>World</strong></p></div>';
  ...
  describe("並行実行の安全性（インスタンス分離）", () => {
    ...
    it("異なる危険要素の混合を正しくカウントする", () => {
      ...
      const mixedMalicious = '<script>evil()</script><div onclick="bad()">click</div><p>safe</p>';
  ...
  describe("ストレステスト: 並行リクエスト", () => {
    it("10件の並行リクエストでデータ競合が発生しない", async () => {
      ...
      // Act: 並行で実行
      const results = await Promise.all(
        requests.map((req) => Promise.resolve(sanitizeHtmlWithDetails(req.html)))
      );
  ...
  describe("SanitizeOptions", () => {
    ...
    it("allowSvg: false でSVG要素を除去する", () => {
      // Arrange
      const html = '<div><svg><circle cx="50" cy="50" r="40"/></svg></div>';
  ...
  describe("エッジケース", () => {
    ...
    it("SVGコンテキスト内のscriptを処理する", () => {
      ...
      const html = "<svg><script>alert(1)</script></svg><p>safe</p>";
  ...
  describe("preserveDocumentStructure オプション", () => {
    ...
    describe("ドキュメント構造の保持", () => {
      it("<html> タグが保持される", () => {
        // Arrange
        const html =
          '<html lang="ja"><head><title>Test</title></head><body><p>Hello</p></body></html>';
    ...
    describe("安全なメタデータタグの保持", () => {
      it('<meta name="description"> が保持される', () => {
        // Arrange
        const html =
          '<html><head><meta name="description" content="Page description"><title>Test</title></head><body><p>Hello</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it('<meta name="viewport"> が保持される', () => {
        // Arrange
        const html =
          '<html><head><meta name="viewport" content="width=device-width, initial-scale=1"><title>Test</title></head><body><p>Hello</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it('<meta charset="utf-8"> が保持される', () => {
        // Arrange
        const html =
          '<html><head><meta charset="utf-8"><title>Test</title></head><body><p>Hello</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
    ...
    describe("セキュリティ維持（preserveDocumentStructure有効時）", () => {
      it("<script> タグを除去する", () => {
        ...
        const html =
          '<html lang="ja"><head><title>Test</title><script>alert("xss")</script></head><body><p>Hello</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it("javascript: URLを除去する", () => {
        // Arrange
        const html =
          '<html lang="ja"><head><title>Test</title></head><body><a href="javascript:alert(1)">Click</a></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it("on* イベントハンドラを除去する", () => {
        // Arrange
        const html =
          '<html lang="ja"><head><title>Test</title></head><body><div onclick="alert(1)">Content</div></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it("<iframe> タグを除去する", () => {
        // Arrange
        const html =
          '<html lang="ja"><head><title>Test</title></head><body><iframe src="http://evil.com"></iframe><p>Safe</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it('<meta http-equiv="refresh"> を除去する（リダイレクト攻撃防止）', () => {
        ...
        const html =
          '<html><head><meta http-equiv="refresh" content="0;url=http://evil.com"><title>Test</title></head><body><p>Hello</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it("<base href> タグを除去する（相対URL操作防止）", () => {
        ...
        const html =
          '<html><head><base href="http://evil.com/"><title>Test</title></head><body><p>Hello</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it('<link rel="stylesheet"> を除去する（外部CSS読み込み防止）', () => {
        ...
        const html =
          '<html><head><link rel="stylesheet" href="http://evil.com/steal.css"><title>Test</title></head><body><p>Hello</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it("<form> タグを除去する", () => {
        // Arrange
        const html =
          '<html lang="ja"><head><title>Test</title></head><body><form action="http://evil.com"><input type="text"></form><p>Safe</p></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
      ...
      it("data:text/html URIスキームを除去する", () => {
        // Arrange
        const html =
          '<html lang="ja"><head><title>Test</title></head><body><a href="data:text/html,<script>alert(1)</script>">Click</a></body></html>';
        const options: SanitizeOptions = { preserveDocumentStructure: true };
...
<body>
  ...
  describe("preStripDangerousTags", () => {
    ...
    describe("コンテンツ付き危険タグの除去", () => {
      ...
      it("scriptタグとそのコンテンツが除去される", () => {
        ...
        const html = makeLargeHtml('<script>alert("xss")</script>', "<p>Safe content</p>");
    ...
    describe("自己閉じ・空タグの除去", () => {
      ...
      it("inputタグ（自己閉じ）が除去される", () => {
        // Arrange
        const html = makeLargeHtml('<input type="text" name="secret" />', "<p>Content</p>");
      ...
      it("metaタグが除去される", () => {
        // Arrange
        const html = makeLargeHtml('<meta name="description" content="test">', "<p>Content</p>");
      ...
      it("linkタグが除去される", () => {
        // Arrange
        const html = makeLargeHtml('<link rel="stylesheet" href="style.css">', "<p>Content</p>");
      ...
      it("baseタグが除去される", () => {
        // Arrange
        const html = makeLargeHtml('<base href="http://evil.com/">', "<p>Content</p>");
    ...
    describe("分析に必要なタグの保持", () => {
      ...
      it("imgタグは保持される", () => {
        // Arrange
        const html = makeLargeHtml('<img src="photo.jpg" alt="Photo" />');
      ...
      it("buttonタグは保持される（CTA検出に必要）", () => {
        ...
        const html = makeLargeHtml('<button type="button">Click Me</button>');
    ...
    describe("大文字/混合ケース対応", () => {
      ...
      it("大文字のSCRIPTタグが除去される", () => {
        // Arrange
        const html = makeLargeHtml('<SCRIPT>alert("xss")</SCRIPT>', "<p>Safe</p>");
      ...
      it("大文字のIFRAMEタグが除去される", () => {
        // Arrange
        const html = makeLargeHtml('<IFRAME src="http://evil.com"></IFRAME>', "<p>Safe</p>");
      ...
      it("大文字のINPUTタグ（自己閉じ）が除去される", () => {
        // Arrange
        const html = makeLargeHtml('<INPUT type="hidden" value="secret" />', "<p>Safe</p>");
  ...
  describe("パフォーマンス", () => {
    it("2MB超のHTMLをsanitizeHtmlが5秒以内に完了する（DOMPurifyバイパス）", () => {
      ...
      // SEC-HIGH-01: イベントハンドラ属性を散在させる
      for (let i = 0; i < 50; i++) {
        segments.push(`<img src="x" onerror="alert(${i})">`);
        segments.push(`<svg onload="alert(${i})">`);
        segments.push(`<a href="javascript:alert(${i})">click</a>`);
      }
      // styleタグ（保持されるべき）
      for (let i = 0; i < 20; i++) {
        segments.push(
          `<style>.component-${i} { background: linear-gradient(to right, #${String(i).padStart(2, "0")}0000, #0000${String(i).padStart(2, "0")}); animation: fade-${i} 1s ease; }</style>`
        );
      }
      ...
      // SEC-HIGH-01: イベントハンドラ属性が除去されている
  ...
  // =============================================
  // SEC-HIGH-01: stripDangerousAttributes テスト
  // =============================================
  describe("stripDangerousAttributes (SEC-HIGH-01)", () => {
    it("double-quoted on* イベントハンドラを除去する", () => {
      const html = '<img src="x" onerror="alert(1)">';
      const result = stripDangerousAttributes(html);
      expect(result).not.toContain("onerror");
      expect(result).toContain('<img src="x"');
    });

    it("single-quoted on* イベントハンドラを除去する", () => {
      const html = "<div onclick='steal()'>";
      const result = stripDangerousAttributes(html);
      expect(result).not.toContain("onclick");
    });

    it("unquoted on* イベントハンドラを除去する", () => {
    ...
    it("SVG onload を除去する", () => {
      const html = '<svg onload="alert(document.cookie)"><circle r="10"/></svg>';
      const result = stripDangerousAttributes(html);
      expect(result).not.toContain("onload");
      expect(result).toContain("<svg");
    });

    it("javascript: URL in href を除去する", () => {
    ...
    it("javascript: URL in src を除去する", () => {
      const html = `<iframe src="javascript:alert('xss')">`;
      const result = stripDangerousAttributes(html);
      expect(result).not.toContain("javascript:");
    });

    it("複数のイベントハンドラを同時に除去する", () => {
      const html = '<div onclick="a()" onmouseover="b()" onload="c()">safe</div>';
      const result = stripDangerousAttributes(html);
    ...
    it("安全な属性は保持する", () => {
      const html =
        '<div class="container" id="main" style="color:red" data-value="1">content</div>';
      const result = stripDangerousAttributes(html);
      expect(result).toContain('class="container"');
      expect(result).toContain('id="main"');
      expect(result).toContain('style="color:red"');
      expect(result).toContain('data-value="1"');
    });

    it("イベントハンドラが無いHTMLはそのまま返す", () => {
