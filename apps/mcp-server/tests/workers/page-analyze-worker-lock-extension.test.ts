...
describe("PageAnalyzeWorker - Lock Extension", () => {
  ...
  /** Strip JSDoc comments (/** ... *​/) to avoid false indexOf matches on phase names in doc blocks */
  function stripJsDocComments(source: string): string {
    return source.replace(/\/\*\*[\s\S]*?\*\//g, "");
  }

  beforeEach(() => {
    // types.ts first (constants + utility definitions),
    // then phase files (implementation, stripped of module doc comments),
    // then orchestrator last (so lastIndexOf('finally') finds the orchestrator's finally block)
    workerSource =
      fs.readFileSync(typesPath, "utf8") +
      "\n" +
      stripJsDocComments(fs.readFileSync(phase2Path, "utf8")) +
      "\n" +
      stripJsDocComments(fs.readFileSync(phase4Path, "utf8")) +
      "\n" +
      stripJsDocComments(fs.readFileSync(phase5Path, "utf8")) +
      "\n" +
  ...
  describe("createLockExtender utility", () => {
    ...
    it("should accept job, token, lockDuration, and intervalMs parameters", () => {
      ...
      const fnMatch = workerSource.match(/function createLockExtender\([^)]+\)/);
  ...
  describe("LockExtender type", () => {
    it("should define LockExtender interface with start and stop methods", () => {
      expect(workerSource).toMatch(/(?:interface|type)\s+LockExtender/);
    });
  ...
  describe("Phase boundary explicit lock extensions", () => {
    it("should call extendJobLock before Scroll Vision phase", () => {
      ...
      const surrounding = workerSource.slice(scrollVisionStart, scrollVisionStart + 300);
    ...
    it("should call extendJobLock before Narrative phase", () => {
      ...
      // extendJobLock is called just after the phase comment header
      const surrounding = workerSource.slice(narrativeStart, narrativeStart + 300);
      expect(surrounding).toContain("extendJobLock");
    });
  ...
  describe("extendJobLock helper", () => {
    ...
    it("should accept job, token, lockDuration, and phaseName parameters", () => {
      const fnMatch = workerSource.match(/async function extendJobLock\([^)]+\)/);
      expect(fnMatch).not.toBeNull();
      const fnSignature = fnMatch![0];
      expect(fnSignature).toContain("job");
      expect(fnSignature).toContain("token");
  ...
  describe("createLockExtender behavior", () => {
    ...
    it("should return LockExtender with start and stop methods", () => {
      ...
      const extender = createLockExtender(mockJob as never, "test-token", 600000, 300000);
    ...
    it("should not throw when extendLock fails", () => {
      ...
      const extender = createLockExtender(mockJob as never, "test-token", 600000, 5000);
