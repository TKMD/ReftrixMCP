...
describe("PageAnalyzeWorker - Responsive Phase (Phase 4.5)", () => {
  // NOTE: Phase 4.5 はリファクタリングで phase-4-narrative.ts に移動済み
  // メインworkerファイル + フェーズモジュールの両方を読み込む
  const workerSourcePath = path.resolve(__dirname, "../../src/workers/page-analyze-worker.ts");
  const phaseSourcePath = path.resolve(__dirname, "../../src/workers/phases/phase-4-narrative.ts");
  ...
  beforeAll(() => {
    ...
    phaseSource = fs.readFileSync(phaseSourcePath, "utf8");

    // Phase 4.5 のコードセクションを抽出（phase-4-narrative.ts から）
    const responsiveStart = phaseSource.indexOf("Phase 4.5: Responsive Analysis");
    expect(responsiveStart).toBeGreaterThan(-1);
    responsiveSection = phaseSource.slice(responsiveStart);
  });
  ...
  describe("Scenario 1: 正常完了 (Happy path)", () => {
    ...
    it("should call validateExternalUrl for SSRF check", () => {
      expect(responsiveSection).toContain("validateExternalUrl");
    });

    it("should call isUrlAllowedByRobotsTxt after SSRF check passes", () => {
      const ssrfCheckPos = responsiveSection.indexOf("validateExternalUrl");
  ...
  describe("Scenario 3: robots.txt blocked", () => {
    ...
    it('should call skipPhase("responsive") with robots.txt reason when blocked', () => {
      expect(responsiveSection).toMatch(/skipPhase\(\s*"responsive",\s*`Robots\.txt blocked:/s);
    });
    ...
    it("should have respectRobotsTxt field in PageAnalyzeJobOptions", () => {
      // Queue定義を読み込んで検証
      const queuePath = path.resolve(__dirname, "../../src/queues/page-analyze-queue.ts");
  ...
  describe("Scenario 5: DB save failure (graceful degradation)", () => {
    ...
    it("should still set results.responsive even when save fails (responsiveAnalysisId is undefined)", () => {
      ...
      expect(responsiveSection).toContain(
  ...
  describe("Scenario 6: memoryAborted skips responsive phase", () => {
    ...
    it("should derive responsiveEnabled from options.responsiveOptions?.enabled !== false", () => {
      ...
      expect(phaseSource).toContain("options.responsiveOptions?.enabled !== false");
    });
  });
  ...
  describe("Responsive options forwarding", () => {
    ...
    it("should guard each option with undefined check before forwarding", () => {
      ...
      for (const field of optionFields) {
        const pattern = new RegExp(`rOpts\\?\\.${field}\\s*!==\\s*undefined`);
