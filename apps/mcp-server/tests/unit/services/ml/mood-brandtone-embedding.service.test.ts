...
describe("generateBatchMoodBrandTone", () => {
  ...
  it("100件のバッチ処理を5秒以内に完了させる", async () => {
    ...
    const manyEmbeddings = Array(100)
      .fill(null)
...
describe("SectionEmbedding DB 統合テスト", () => {
  ...
  beforeEach(() => {
    ...
    const mockPrisma: MockPrismaClient = {
      ...
      $transaction: vi
        .fn()
        .mockImplementation((callback: (prisma: MockPrismaClient) => Promise<unknown>) =>
          callback(mockPrisma)
        ),
  ...
  it("Mood と BrandTone の両方を同時に保存する", async () => {
    ...
    const brandToneResult = await service.generateBrandToneEmbedding(
      sampleBrandToneTextRepresentation
    );
  ...
  it("保存エラー時は適切にハンドルされる", async () => {
    ...
    await expect(saveMoodBrandToneEmbedding("section-1", { mood: moodResult })).rejects.toThrow();
  });
});
...
describe("エラーハンドリング", () => {
  ...
  it("Embedding サービスのタイムアウトをキャッチする", async () => {
    ...
    await expect(service.generateMoodEmbedding(sampleMoodTextRepresentation)).rejects.toThrow();
  });
