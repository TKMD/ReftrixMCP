...
describe("Tailwind CSS最適化", () => {
  describe("convertHtmlToJsx with useTailwind option", () => {
    ...
    it("複数のスタイルプロパティを変換", () => {
      const html =
        '<div style="padding: 20px; background-color: white; display: flex;">Content</div>';
      const result = convertHtmlToJsx(html, { useTailwind: true });
      expect(result).toContain("p-5");
      expect(result).toContain("bg-white");
  ...
  describe("複合変換テスト", () => {
    it('期待される変換: style={{padding: "20px", backgroundColor: "#fff"}} → className="p-5 bg-white"', () => {
      const html =
        '<div style="padding: 20px; background-color: #fff;"><div>Hero Section</div></div>';
      const result = convertHtmlToJsx(html, { useTailwind: true });
      expect(result).toContain("p-5");
      expect(result).toContain("bg-white");
      expect(result).not.toContain("style={{");
    ...
    it("完全な変換後はstyle属性が残らない", () => {
      const html =
        '<div style="display: flex; justify-content: center; align-items: center; padding: 16px; background-color: white;">Content</div>';
      const result = convertHtmlToJsx(html, { useTailwind: true });
      expect(result).toContain("flex");
