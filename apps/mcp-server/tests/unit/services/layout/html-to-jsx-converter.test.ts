  ...
  describe("convertHtmlToJsx", () => {
    ...
    describe("自己閉じタグの変換", () => {
      ...
      it("col要素を自己閉じタグに変換できる", () => {
        ...
        const html =
          '<table><colgroup><col span="2"></colgroup><tbody><tr><td>Cell</td></tr></tbody></table>';
        const result = convertHtmlToJsx(html);
    ...
    describe("イベントハンドラ属性の変換", () => {
      ...
      it("複数のイベントハンドラをすべて除去できる", () => {
        const html = '<button onclick="fn1()" onmouseover="fn2()" onfocus="fn3()">Button</button>';
        const result = convertHtmlToJsx(html);
        expect(result).not.toContain("onclick=");
        expect(result).not.toContain("onmouseover=");
    ...
    describe("data-*, aria-* 属性は変換しない", () => {
      ...
      it("aria-*属性はそのまま残す", () => {
        const html = '<button aria-label="Close" aria-expanded="false">X</button>';
        const result = convertHtmlToJsx(html);
    ...
    describe("TailwindCSS統合", () => {
      ...
      it("useTailwind: trueで変換不可スタイルはstyleとして残す", () => {
        const html =
          '<div style="display: flex; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">Content</div>';
        const options: HtmlToJsxOptions = { useTailwind: true };
        const result = convertHtmlToJsx(html, options);
