...
const BASE_CONFIDENCE: Record<DetectionSource, number> = {
  ...
  web_animations_api: 0.9, // Web Animations APIも高信頼
  library_signature: 0.75, // ライブラリシグネチャは中程度
  raf_monitoring: 0.7, // RAF監視は間接的
...
function mergeConfidenceScores(sources: DetectionSource[]): number {
  ...
  const sortedScores = sources.map((s) => BASE_CONFIDENCE[s]).sort((a, b) => b - a);
...
describe("JSアニメーション検出 - 信頼度スコア", () => {
  describe("単一ソースの信頼度", () => {
    ...
    it("Web Animations APIソースは0.90の信頼度を持つ", () => {
      ...
      expect(confidence.sourceScores.web_animations_api).toBe(0.9);
    });
  ...
  describe("複数ソースからの信頼度マージ", () => {
    ...
    it("3つ以上のソースでも適切にマージされる", () => {
      const confidence = calculateConfidence(["cdp", "web_animations_api", "library_signature"]);
    ...
    it("低信頼度ソースのみでは信頼度は低い", () => {
      const confidence = calculateConfidence(["raf_monitoring", "intersection_observer"]);
  ...
  describe("信頼度情報の完全性", () => {
    ...
    it("追加根拠が含まれる", () => {
      const confidence = calculateConfidence(
        ["cdp"],
        ["Animation duration: 1000ms", "Keyframes detected: 3"]
      );
...
describe("アニメーション検出結果の信頼度", () => {
  ...
  describe("スクロールトリガーアニメーション", () => {
    ...
    it("IO + Web Animations APIで信頼度上昇", () => {
      ...
      expect(result.confidence.score).toBeGreaterThan(0.9);
    });
  });
  ...
  describe("RAFベースアニメーション", () => {
    it("RAF監視のみは中程度の信頼度", () => {
      ...
      expect(result.confidence.score).toBe(0.7);
    });
    ...
    it("RAF + DOM変更監視で信頼度上昇", () => {
      const result: AnimationDetectionWithConfidence = {
        ...
        confidence: calculateConfidence(
          ["raf_monitoring"],
          ["DOM mutations detected during RAF callback", "Transform property changes: 60 times"]
        ),
        details: {
          callCount: 60,
          avgFrameTime: 16.67,
      ...
      expect(result.confidence.score).toBe(0.7); // ソースは1つなのでスコアは変わらない
...
describe("検出漏れパターンの信頼度補正", () => {
  describe("page.setContent使用時のCDP検出", () => {
    it("CDP検出が不安定な場合、Web Animations APIで補完", () => {
      ...
      expect(confidence.score).toBeGreaterThanOrEqual(0.9);
    });
  });
  ...
  describe("遅延ロードアニメーション", () => {
    it("waitTime後に検出されたアニメーションは信頼度が維持される", () => {
      ...
      const confidence = calculateConfidence(
        ["cdp"],
        ["Detected after 500ms wait", "Animation may have lazy-started"]
      );
  ...
  describe("サードパーティスクリプト由来", () => {
    it("サードパーティライブラリの検出は信頼度にラベル付け", () => {
      const confidence = calculateConfidence(
        ["library_signature"],
        ["Third-party library: GSAP", "Version: 3.12.0"]
      );
