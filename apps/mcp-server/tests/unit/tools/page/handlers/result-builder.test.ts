...
const createFullQualityResult = (): QualityServiceResult => ({
  ...
  axisGrades: {
    originality: "B" as const,
    craftsmanship: "A" as const,
    contextuality: "A" as const,
  },
...
describe("result-builder", () => {
  ...
  describe("determineErrorCode", () => {
    it("タイムアウトエラーメッセージを正しく判定すること", () => {
      ...
      expect(determineErrorCode("Request Timeout")).toBe(PAGE_ANALYZE_ERROR_CODES.TIMEOUT_ERROR);
    });

    it("HTTPエラーメッセージを正しく判定すること", () => {
      expect(determineErrorCode("HTTP 404 Not Found")).toBe(PAGE_ANALYZE_ERROR_CODES.HTTP_ERROR);
      expect(determineErrorCode("Page Not Found")).toBe(PAGE_ANALYZE_ERROR_CODES.HTTP_ERROR);
    });
    ...
    it("その他のエラーメッセージをネットワークエラーとして判定すること", () => {
      ...
      expect(determineErrorCode("Connection refused")).toBe(PAGE_ANALYZE_ERROR_CODES.NETWORK_ERROR);
    });
  });
  ...
  describe("buildMotionResult", () => {
    ...
    describe("エラーケース", () => {
      ...
      it("webgl_animation_error フィールドを含むこと", () => {
        ...
        expect((result as MotionServiceResult).webgl_animation_error).toBe(
  ...
  describe("extractActionableWarning", () => {
    ...
    it("未知のエラーをレガシー形式に変換すること", async () => {
      const { legacyWarningToActionable } =
        await import("../../../../../src/utils/actionable-warning");
