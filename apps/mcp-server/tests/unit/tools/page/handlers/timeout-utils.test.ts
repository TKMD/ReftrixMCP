...
describe("withTimeoutGraceful", () => {
  ...
  it("タイムアウト時はnullを返し警告を追加すること", async () => {
    ...
    const resultPromise = withTimeoutGraceful(promise, 100, "slow layout", "layout", warnings);
  ...
  it("motion featureでのタイムアウトが正しく処理されること", async () => {
    ...
    const resultPromise = withTimeoutGraceful(promise, 200, "motion detection", "motion", warnings);
  ...
  it("タイムアウト以外のエラーもGraceful Degradationで処理されること", async () => {
    ...
    const result = await withTimeoutGraceful(promise, 1000, "layout analysis", "layout", warnings);
  ...
  it("motion featureのエラーは正しいエラーコードを使用すること", async () => {
    ...
    await withTimeoutGraceful(promise, 1000, "motion detection", "motion", warnings);
  ...
  it("quality featureのエラーは正しいエラーコードを使用すること", async () => {
    ...
    await withTimeoutGraceful(promise, 1000, "quality evaluation", "quality", warnings);
...
describe("withTimeoutAndTracking", () => {
  ...
  it("Strict戦略で非タイムアウトエラー時は例外を再スローすること", async () => {
    ...
    await expect(
      withTimeoutAndTracking(promise, 1000, "error phase", "motion", tracker, warnings)
