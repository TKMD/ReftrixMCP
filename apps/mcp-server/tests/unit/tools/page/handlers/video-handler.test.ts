...
const MOCK_FRAME_ANALYSIS_RESULT = {
  ...
  motionVectors: [
    ...
    {
      frameRange: "frame-0010.png to frame-0020.png",
      dx: 10,
      dy: 0,
      magnitude: "10.0",
      direction: "right",
...
describe("Video Handler Unit Tests", () => {
  ...
  describe("Frame Capture Enabled", () => {
    it("should execute frame capture when enabled", async () => {
      ...
      // Assert
      expect(mockExecuteFrameCapture).toHaveBeenCalledWith(
        TEST_URL,
  ...
  describe("Result Transformation", () => {
    ...
    it("should correctly extract max_diff from animation zones", async () => {
      ...
      // Assert
      // max_diff = 20.0% = 0.20
      expect(result.frame_analysis?.summary.max_diff).toBeCloseTo(0.2, 2);
    });
