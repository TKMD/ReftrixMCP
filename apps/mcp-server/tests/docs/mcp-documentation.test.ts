...
describe("MCP Documentation Completeness", () => {
  ...
  describe("必須ドキュメントの存在確認", () => {
    ...
    it("MCPツールセクションが存在すること", () => {
      // 日本語または英語のセクション名を許容
      const hasMCPTools =
        readmeContent.includes("MCP Tools") ||
    ...
    it("インストールセクションが存在すること", () => {
      ...
      const hasInstallation =
        readmeContent.includes("Installation") || readmeContent.includes("インストール");
  ...
  describe("layout.search ツールドキュメント", () => {
    ...
    it("layout.searchのレスポンス形式が説明されていること", () => {
      ...
      const hasResponseFormat =
        readmeContent.includes("limit") && readmeContent.includes("sectionType");
  ...
  describe("Claude Desktop設定例", () => {
    ...
    it("環境変数設定例が含まれること", () => {
      ...
      const hasEnvVars = readmeContent.includes("env") || readmeContent.includes("NODE_ENV");
