...
describe("P1-SEC-INIT-01: 本番環境での全必須カテゴリ初期化強制", () => {
  ...
  describe("assertProductionRequiredCategoriesInitialized", () => {
    describe("開発環境での動作", () => {
      it("開発環境では必須カテゴリ不足でもエラーにならない", () => {
        ...
        expect(() => assertProductionRequiredCategoriesInitialized(["motion"])).not.toThrow();
      });

      it("開発環境で空のカテゴリリストでもエラーにならない", () => {
        process.env.NODE_ENV = "development";

        expect(() => assertProductionRequiredCategoriesInitialized([])).not.toThrow();
      });
    ...
    describe("本番環境での動作", () => {
      ...
      it("全必須カテゴリが初期化されていればエラーにならない", () => {
        ...
        expect(() => assertProductionRequiredCategoriesInitialized(allCategories)).not.toThrow();
      });
      ...
      it("motionカテゴリが欠けている場合はエラー", () => {
        ...
        expect(() => assertProductionRequiredCategoriesInitialized(missingMotion)).toThrow(
          ProductionCategoryRequiredError
        );
      ...
      it("layoutカテゴリが欠けている場合はエラー", () => {
        ...
        expect(() => assertProductionRequiredCategoriesInitialized(missingLayout)).toThrow(
          ProductionCategoryRequiredError
        );
      ...
      it("qualityカテゴリが欠けている場合はエラー", () => {
        ...
        expect(() => assertProductionRequiredCategoriesInitialized(missingQuality)).toThrow(
          ProductionCategoryRequiredError
        );
      ...
      it("pageカテゴリが欠けている場合はエラー", () => {
        ...
        expect(() => assertProductionRequiredCategoriesInitialized(missingPage)).toThrow(
          ProductionCategoryRequiredError
        );
      ...
      it("複数カテゴリが欠けている場合は全て報告される", () => {
        ...
        expect(() => assertProductionRequiredCategoriesInitialized(missingMultiple)).toThrow(
          ProductionCategoryRequiredError
        );
      ...
      it("空のカテゴリリストはエラー", () => {
        expect(() => assertProductionRequiredCategoriesInitialized([])).toThrow(
          ProductionCategoryRequiredError
        );
  ...
  describe("セキュリティ統合テスト", () => {
    it("本番環境 + 部分初期化 = 起動失敗", () => {
      ...
      expect(() => assertProductionRequiredCategoriesInitialized(["motion", "layout"])).toThrow(
        ProductionCategoryRequiredError
      );
    });
