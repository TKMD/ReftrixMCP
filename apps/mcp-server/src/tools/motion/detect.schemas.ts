 ...
 */
export const webAnimationKeyframeSchema = z
  .object({
...
export const frameImageAnalysisInputOptionsSchema = z.object({
  ...
  frame_dir: z
    ...
    .optional()
    .describe(
      'Directory containing frame images. Falls back to frameCaptureResult.output_dir, frame_capture_options.output_dir, or "/tmp/reftrix-frames/".'
    ),
  ...
  sample_interval: z
    ...
    .default(10)
    .describe(
      "Analyze every Nth frame. Higher values = faster but less precise. (1-100, default: 10)"
    ),
  ...
  diff_threshold: z
    ...
    .default(0.1)
    .describe(
      "Pixel diff threshold (0-1). Frame pairs exceeding this are marked as significant changes. (default: 0.1)"
    ),
  ...
  cls_threshold: z
    ...
    .default(0.05)
    .describe(
      'Layout shift (CLS) threshold. Core Web Vitals considers 0.05 as "good". (default: 0.05)'
    ),
  ...
  motion_threshold: z
    ...
    .default(50)
    .describe(
      "Motion detection threshold in pixels. Movements exceeding this are tracked as motion vectors. (default: 50)"
    ),
   ...
   * @default true
   */
  parallel: z.boolean().default(true).describe("Enable parallel frame processing. (default: true)"),
});
...
export const lighthouseOptionsSchema = z.object({
  ...
  categories: z.array(lighthouseCategorySchema).default(["performance"]).optional(),
...
export const motionDetectInputSchema = z
  .object({
    ...
    html: z
      ...
      .optional(),
    css: z.string().max(5_000_000, { message: "CSSコンテンツは5MB以下にしてください" }).optional(),
    ...
    baseUrl: z.string().url({ message: "baseUrlは有効なURL形式である必要があります" }).optional(),
  ...
  .refine(
    ...
    {
      message:
        "fetchExternalCssがtrueの場合、baseUrlは必須です（css+html/pageIdまたはvideo/runtime/hybrid+url指定時を除く）",
      path: ["baseUrl"],
    }
  );
...
export const motionDetectDataSchema = z.object({
   ...
   */
  js_animations: jsAnimationResultSchema
    .nullable()
    .optional()
    .describe(
...
export type MotionWarningCode = (typeof MOTION_WARNING_CODES)[keyof typeof MOTION_WARNING_CODES];
 ...
 * パフォーマンスレベルを計算
 */
export function calculatePerformanceLevel(info: PerformanceInfo): PerformanceLevel {
  // GPU加速プロパティのみ使用 && レイアウト/ペイントなし = excellent
  if ((info.usesTransform || info.usesOpacity) && !info.triggersLayout && !info.triggersPaint) {
    return "excellent";
  }
...
export function calculateComplexityScore(patterns: MotionPattern[]): number {
  ...
  // キーフレーム複雑度
  const keyframeCount = patterns.reduce((sum, p) => sum + (p.keyframes?.length ?? 0), 0);
  score += Math.min(keyframeCount * 2, 20);

  // プロパティ数（propertiesがない場合も安全に処理）
  const propertyCount = patterns.reduce((sum, p) => sum + (p.properties?.length ?? 0), 0);
 ...
 * タイプ別集計
 */
export function countByType(patterns: MotionPattern[]): Record<MotionType, number> {
 ...
 * トリガー別集計
 */
export function countByTrigger(patterns: MotionPattern[]): Record<TriggerType, number> {
 ...
 * カテゴリ別集計
 */
export function countByCategory(patterns: MotionPattern[]): Record<MotionCategory, number> {
 ...
 * @returns 計算済みの設定
 */
export function calculateFrameCaptureConfig(options: FrameCaptureOptions = {}): FrameCaptureConfig {
  // デフォルト値
...
export function generateFrameFileInfos(config: FrameCaptureConfig): FrameFileInfo[] {
  ...
  for (let i = 0; i < config.total_frames; i++) {
    ...
    const scroll_position_px = Math.min(i * config.scroll_px_per_frame, config.page_height_px);
