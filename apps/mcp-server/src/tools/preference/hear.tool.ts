 ...
 */
export async function preferenceHearHandler(
  input: unknown
): Promise<PreferenceHearOutput> {
  if (isDevelopment()) {
  ...
  try {
    ...
    if (validated.feedback && validated.feedback.length > 0 && validated.preference_text && validated.profile_id) {
...
export const preferenceHearToolDefinition = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      feedback: {
        type: 'array',
        description: 'フィードバック配列（存在する場合はモードB） / Feedback array (Mode B if present)',
        items: {
          type: 'object',
          properties: {
            sample_id: {
      ...
      exclude_ids: {
        ...
        maxItems: 50,
        description: '除外するサンプルID配列（既評価済み） / Sample IDs to exclude (already evaluated)',
      },
    },
  },
};
