...
function createErrorResponse(
  ...
  return {
    ...
    error: {
      ...
      message,
      suggestion:
        suggestion ?? "パラメータなしで呼び出して利用可能なパレット一覧を確認してください。",
    },
  };
}
 ...
 */
export async function styleGetPaletteHandler(input: unknown): Promise<StyleGetPaletteResponse> {
  if (isDevelopment()) {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      mode: {
        ...
        default: "both",
        description: "Filter by palette mode. light/dark/both (default: both)",
      },
      include_gradients: {
        type: "boolean",
        default: true,
        description: "Include gradient info when ID specified (default: true)",
      },
