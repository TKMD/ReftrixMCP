 ...
 */
const LARGE_FONT_SIZE_RESPONSIVE_MAP: Record<number, { mobile: string; md: string; lg: string }> = {
 ...
 * 元のpx値 → [mobile, md, lg]のクラス配列
 */
const LARGE_PADDING_RESPONSIVE_MAP: Record<number, { mobile: string; md: string; lg: string }> = {
  // 32px → p-4 / p-6 / p-8
...
function convertGridTemplateColumns(value: string): string | null {
  ...
  // repeat(N, 1fr) パターン
  const repeatMatch = normalizedValue.match(
    /^repeat\s*\(\s*(\d+)\s*,\s*(1fr|minmax\s*\(\s*0\s*,\s*1fr\s*\))\s*\)$/
  );
 ...
 */
function convertSpacing(value: string, prefix: string): string | null {
  const spacingValue = convertSpacingValue(value);
  if (spacingValue !== null) {
    return `${prefix}-${spacingValue}`;
  }
 ...
 * @returns 変換されたクラス配列、または null
 */
function convertLargePaddingResponsive(pixels: number, prefix: string = "p"): string[] | null {
